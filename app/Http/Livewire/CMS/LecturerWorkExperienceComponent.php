...
class LecturerWorkExperienceComponent extends Component
{
    use WithPagination;

    public $menu_name = "Lecturer Work Experience";
    public $menu_icon = "bi bi-briefcase";
    public $menu_slug = "lecturer-work-experience";
    public $menu_table = "lecturer_work_experience";
    ...
    public $lecturer_work_experience;
    public $lecturer = "";
    public $name;
    public $name_id;
    public $description;
    ...
    public function resetFilter()
    {
        ...
        $this->lecturer_work_experience = null;
        $this->lecturer = "";
        $this->name = null;
        $this->name_id = null;
    ...
    {
        $this->active = $this->lecturer_work_experience->active;
        $this->lecturer = $this->lecturer_work_experience->lecturer?->id;
        $this->name = $this->lecturer_work_experience->name;
        $this->name_id = $this->lecturer_work_experience->name_id;
        $this->description = $this->lecturer_work_experience->description;
        $this->description_id = $this->lecturer_work_experience->description_id;
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != "index" || $this->menu_type != "trash")) {
            if ($this->menu_type == "view") {
                $this->lecturer_work_experience = LecturerWorkExperience::withTrashed()->find($this->row);
            } else {
                $this->lecturer_work_experience = LecturerWorkExperience::find($this->row);
            }

            if (!$this->lecturer_work_experience) {
    ...
    public function form($menu_type, $id)
    {
        ...
        if ($menu_type != "add" && $id) {
            $this->lecturer_work_experience = LecturerWorkExperience::find($id);
    ...
    public function view($id)
    {
        ...
        $this->lecturer_work_experience = LecturerWorkExperience::withTrashed()->find($id);
    ...
    {
        $id = $this->menu_type == "edit" ? $this->lecturer_work_experience->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->menu_type == "add" || $this->menu_type == "clone") {
            $this->lecturer_work_experience = new LecturerWorkExperience();
            if (env("APP_ENV") != "testing") {
                DB::statement(DB::raw("ALTER TABLE {$this->menu_table} AUTO_INCREMENT = 1"));
            }
        }

        $this->lecturer_work_experience->active = $this->active;

        $this->lecturer_work_experience->lecturer_id = $this->lecturer;
        $this->lecturer_work_experience->name = $this->name;
        $this->lecturer_work_experience->name_id = $this->name_id;
        $this->lecturer_work_experience->description = Str::of(htmlspecialchars($this->description))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->lecturer_work_experience->description_id = Str::of(htmlspecialchars($this->description_id))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->lecturer_work_experience->save();
    ...
    public function active($id)
    {
        ...
        $this->lecturer_work_experience->active = true;
        $this->lecturer_work_experience->save();
        $this->lecturer_work_experience->refresh();
    ...
    public function nonActive($id)
    {
        ...
        $this->lecturer_work_experience->active = false;
        $this->lecturer_work_experience->save();
        $this->lecturer_work_experience->refresh();
    ...
    public function delete($id)
    {
        ...
        $this->lecturer_work_experience->delete();
        $this->lecturer_work_experience->refresh();
    ...
    {
        $this->lecturer_work_experience = LecturerWorkExperience::onlyTrashed()->find($id);

        if (!$this->lecturer_work_experience) {
            return Session::flash("danger", trans("page.{$this->menu_name}") . " " . trans("message.not found or has been deleted"));
        }

        $this->lecturer_work_experience->restore();
        $this->lecturer_work_experience->refresh();
    ...
    public function deletePermanent($id)
    {
        ...
        $this->lecturer_work_experience->forceDelete();
        $this->lecturer_work_experience->refresh();
    ...
    {
        LecturerWorkExperience::onlyTrashed()->restore();
    ...
    {
        LecturerWorkExperience::onlyTrashed()->forceDelete();
    ...
    {
        $created_by = LecturerWorkExperience::groupBy("created_by")->onlyActive()->pluck("created_by");
        return Admin::whereIn("id", $created_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataUpdatedBy()
    {
        $updated_by = LecturerWorkExperience::groupBy("updated_by")->onlyActive()->pluck("updated_by");
        return Admin::whereIn("id", $updated_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataDeletedBy()
    {
        $deleted_by = LecturerWorkExperience::groupBy("deleted_by")->onlyActive()->pluck("deleted_by");
    ...
    public function getDataLecturerWorkExperience()
    {
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_lecturer_work_experience = LecturerWorkExperience::query()
                ...
                if ($this->created_by || $this->created_by == "0") {
                    $data_lecturer_work_experience->where("created_by", $this->created_by);
                }
                if ($this->updated_by || $this->updated_by == "0") {
                    $data_lecturer_work_experience->where("updated_by", $this->updated_by);
                }
                if ($this->deleted_by || $this->deleted_by == "0") {
                    $data_lecturer_work_experience->where("deleted_by", $this->deleted_by);
                }
                if ($this->active || $this->active == "0") {
                    $data_lecturer_work_experience->where("active", $this->active);
                }

                if ($this->order_by == "created_by") {
                    $data_lecturer_work_experience->join("admin", "admin.id", "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "updated_by") {
                    $data_lecturer_work_experience->join("admin", "admin.id", "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "deleted_by") {
                    $data_lecturer_work_experience->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "lecturer_id") {
                    $data_lecturer_work_experience->join("lecturer", "lecturer.id", "{$this->menu_table}.lecturer_id")
                        ->select("{$this->menu_table}.*", "lecturer.name as lecturer_name")
                        ->orderByRaw("lecturer_name {$this->sort_by}");
                } else {
                    $data_lecturer_work_experience->orderBy($this->order_by ?? "id", $this->sort_by ?? "desc");
                }

                if ($this->menu_type == "trash") {
                    $data_lecturer_work_experience->onlyTrashed();
                }

                return $data_lecturer_work_experience->paginate($this->per_page ?? 10);
        }
    }
    ...
    public function render()
    {
        return view("{$this->sub_domain}.livewire.{$this->menu_slug}.index", [
            ...
            "data_lecturer_work_experience" => $this->getDataLecturerWorkExperience(),
