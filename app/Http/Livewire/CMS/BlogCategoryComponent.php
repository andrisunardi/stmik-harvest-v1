...
class BlogCategoryComponent extends Component
{
    ...
    {
        $created_by = BlogCategory::groupBy("created_by")->onlyActive()->pluck("created_by");
        return Admin::whereIn("id", $created_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataUpdatedBy()
    {
        $updated_by = BlogCategory::groupBy("updated_by")->onlyActive()->pluck("updated_by");
        return Admin::whereIn("id", $updated_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataDeletedBy()
    {
        $deleted_by = BlogCategory::groupBy("deleted_by")->onlyActive()->pluck("deleted_by");
        ...
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_blog_category = BlogCategory::query()
                ...
                if ($this->created_by || $this->created_by == "0") {
                    $data_blog_category->where("created_by", $this->created_by);
                }
                if ($this->updated_by || $this->updated_by == "0") {
                    $data_blog_category->where("updated_by", $this->updated_by);
                }
                if ($this->deleted_by || $this->deleted_by == "0") {
                    $data_blog_category->where("deleted_by", $this->deleted_by);
                }
