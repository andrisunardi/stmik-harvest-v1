...
class EventCategoryComponent extends Component
{
    ...
    public $menu_name = 'Event Category';

    public $menu_icon = 'bi bi-tags';

    public $menu_slug = 'event-category';

    public $menu_table = 'event_category';
    ...
    public function resetForm()
    {
        if ($this->event_category) {
            $this->active = $this->event_category->active;
            $this->name = $this->event_category->name;
            $this->name_id = $this->event_category->name_id;
            $this->description = $this->event_category->description;
            $this->description_id = $this->event_category->description_id;
        }
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != 'index' || $this->menu_type != 'trash')) {
            if ($this->menu_type == 'view') {
                $this->event_category = EventCategory::withTrashed()->find($this->row);
            } else {
                $this->event_category = EventCategory::find($this->row);
            }
    ...
    public function form($menu_type, $id)
    {
        ...
        if ($menu_type != 'add' && $id) {
            $this->event_category = EventCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->event_category = EventCategory::withTrashed()->find($id);
    ...
    public function submit()
    {
        ...
        $this->event_category->active = $this->active;

        $this->event_category->name = $this->name;
        $this->event_category->name_id = $this->name_id;
        $this->event_category->description = Str::of(htmlspecialchars($this->description))->swap(['&lt;' => '<', '&gt;' => '>']);
        $this->event_category->description_id = Str::of(htmlspecialchars($this->description_id))->swap(['&lt;' => '<', '&gt;' => '>']);
        $this->event_category->slug = Str::slug($this->name);
        $this->event_category->save();
    ...
    public function active($id)
    {
        ...
        $this->event_category->active = true;
        $this->event_category->save();
        $this->event_category->refresh();
    ...
    public function nonActive($id)
    {
        ...
        $this->event_category->active = false;
        $this->event_category->save();
        $this->event_category->refresh();
    ...
    {
        $this->event_category = EventCategory::onlyTrashed()->find($id);

        if (! $this->event_category) {
            return Session::flash('danger', trans('index.'.Str::slug($this->menu_name, '_')).' '.trans('index.not_found_or_has_been_deleted'));
        }

        $this->event_category->restore();
        $this->event_category->refresh();
    ...
    public function deletePermanent($id)
    {
        ...
        $this->event_category->forceDelete();
        $this->event_category->refresh();
    ...
    {
        EventCategory::onlyTrashed()->restore();
    ...
    {
        EventCategory::onlyTrashed()->forceDelete();
    ...
    {
        $created_by = EventCategory::groupBy('created_by_id')->active()->pluck('created_by_id');
    ...
    {
        $updated_by = EventCategory::groupBy('updated_by_id')->active()->pluck('updated_by_id');
    ...
    {
        $deleted_by = EventCategory::groupBy('deleted_by_id')->active()->pluck('deleted_by_id');
    ...
    {
        if ($this->menu_type == 'index' || $this->menu_type == 'trash') {
            $data_event_category = EventCategory::query()
            ...
            if ($this->created_by_id || $this->created_by_id == '0') {
                $data_event_category->where('created_by_id', $this->created_by_id);
            }
            if ($this->updated_by_id || $this->updated_by_id == '0') {
                $data_event_category->where('updated_by_id', $this->updated_by_id);
            }
            if ($this->deleted_by_id || $this->deleted_by_id == '0') {
                $data_event_category->where('deleted_by_id', $this->deleted_by_id);
            }
            if ($this->active || $this->active == '0') {
                $data_event_category->where('active', $this->active);
            }

            if ($this->order_by == 'created_by_id') {
                $data_event_category->join('admin', 'admin.id', "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", 'admin.name as admin_name')
                        ->orderByRaw("admin_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $data_event_category->join('admin', 'admin.id', "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", 'admin.name as admin_name')
                        ->orderByRaw("admin_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $data_event_category->join('admin', 'admin.id', "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", 'admin.name as admin_name')
                        ->orderByRaw("admin_name {$this->sort_by}");
            } else {
                $data_event_category->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->menu_type == 'trash') {
                $data_event_category->onlyTrashed();
            }

            return $data_event_category->paginate($this->per_page ?? 10);
        }
    }
