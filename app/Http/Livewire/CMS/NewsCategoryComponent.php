...
class NewsCategoryComponent extends Component
{
    ...
    public $menu_name = "News Category";
    public $menu_icon = "bi bi-tags";
    public $menu_slug = "news-category";
    public $menu_table = "news_category";
    ...
    {
        $this->active = $this->news_category->active;
        $this->name = $this->news_category->name;
        $this->name_id = $this->news_category->name_id;
        $this->description = $this->news_category->description;
        $this->description_id = $this->news_category->description_id;
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != "index" || $this->menu_type != "trash")) {
            if ($this->menu_type == "view") {
                $this->news_category = NewsCategory::withTrashed()->find($this->row);
            } else {
                $this->news_category = NewsCategory::find($this->row);
            }
    ...
    public function form($menu_type, $id)
    {
        ...
        if ($menu_type != "add" && $id) {
            $this->news_category = NewsCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->news_category = NewsCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->menu_type == "edit" ? $this->news_category->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->menu_type == "add" || $this->menu_type == "clone") {
            $this->news_category = new NewsCategory();
            if (env("APP_ENV") != "testing") {
                DB::statement(DB::raw("ALTER TABLE {$this->menu_table} AUTO_INCREMENT = 1"));
            }
        }

        $this->news_category->active = $this->active;

        $this->news_category->name = $this->name;
        $this->news_category->name_id = $this->name_id;
        $this->news_category->description = Str::of(htmlspecialchars($this->description))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->news_category->description_id = Str::of(htmlspecialchars($this->description_id))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->news_category->save();
    ...
    public function active($id)
    {
        ...
        $this->news_category->active = true;
        $this->news_category->save();
        $this->news_category->refresh();
    ...
    public function nonActive($id)
    {
        ...
        $this->news_category->active = false;
        $this->news_category->save();
        $this->news_category->refresh();
    ...
    {
        $this->news_category = NewsCategory::onlyTrashed()->find($id);

        if (!$this->news_category) {
            return Session::flash("danger", trans("page.{$this->menu_name}") . " " . trans("message.not found or has been deleted"));
        }

        $this->news_category->restore();
        $this->news_category->refresh();
    ...
    public function deletePermanent($id)
    {
        ...
        $this->news_category->forceDelete();
        $this->news_category->refresh();
    ...
    {
        NewsCategory::onlyTrashed()->forceDelete();
        ...
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_news_category = NewsCategory::query()
                ...
                if ($this->created_by || $this->created_by == "0") {
                    $data_news_category->where("created_by", $this->created_by);
                }
                if ($this->updated_by || $this->updated_by == "0") {
                    $data_news_category->where("updated_by", $this->updated_by);
                }
                if ($this->deleted_by || $this->deleted_by == "0") {
                    $data_news_category->where("deleted_by", $this->deleted_by);
                }
                if ($this->active || $this->active == "0") {
                    $data_news_category->where("active", $this->active);
                }

                if ($this->order_by == "created_by") {
                    $data_news_category->join("admin", "admin.id", "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "updated_by") {
                    $data_news_category->join("admin", "admin.id", "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "deleted_by") {
                    $data_news_category->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else {
                    $data_news_category->orderBy($this->order_by ?? "id", $this->sort_by ?? "desc");
                }

                if ($this->menu_type == "trash") {
                    $data_news_category->onlyTrashed();
                }

                return $data_news_category->paginate($this->per_page ?? 10);
        }
    }
    ...
    public function render()
    {
        return view("{$this->sub_domain}.livewire.{$this->menu_slug}.index", [
            ...
            "data_news_category" => $this->getDataNewsCategory(),
