...
use App\Exports\Finance\AdministrativeCostExport;
use App\Http\Livewire\CMS\Component;
use App\Models\AdministrativeCost;
use App\Models\Bank;
use App\Models\User;
use App\Services\AdministrativeCostService;
use Illuminate\Support\Str;
use Maatwebsite\Excel\Facades\Excel;
use PDF;

class AdministrativeCostComponent extends Component
{
    public $pageName;
    ...
    {
        $this->pageName = 'Administrative Cost';
    ...
    {
        if ($this->administrativeCost) {
            $this->bank_id = $this->bank_id ?: $this->administrativeCost->bank_id;
            $this->amount = $this->amount ?: $this->administrativeCost->amount;
            $this->date = $this->date ?: $this->administrativeCost->date->format('Y-m-d');
            $this->notes = $this->notes ?: $this->administrativeCost->notes;
            $this->is_active = $this->is_active ?: $this->administrativeCost->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->administrativeCost = AdministrativeCost::withTrashed()->findOrFail($this->row);
            } else {
                $this->administrativeCost = AdministrativeCost::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->administrativeCost = AdministrativeCost::find($id);
    ...
    public function view($id)
    {
        ...
        $this->administrativeCost = AdministrativeCost::withTrashed()->find($id);
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new AdministrativeCostService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new AdministrativeCostService())->clone($this->validate(), $this->administrativeCost);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new AdministrativeCostService())->edit($this->administrativeCost, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        $administrativeCost = AdministrativeCost::find($id);

        if (! $administrativeCost) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new AdministrativeCostService())->active($administrativeCost);

        $active = Str::slug(Str::active($administrativeCost->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new AdministrativeCostService())->delete($administrativeCost);
    ...
    public function restore($id)
    {
        ...
        $administrativeCost = AdministrativeCost::onlyTrashed()->find($id);

        if (! $administrativeCost) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new AdministrativeCostService())->restore($administrativeCost);
    ...
    public function deletePermanent($id)
    {
        ...
        (new AdministrativeCostService())->deletePermanent($administrativeCost);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new AdministrativeCostService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new AdministrativeCostService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = AdministrativeCost::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = AdministrativeCost::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = AdministrativeCost::groupBy('deleted_by_id')->pluck('deleted_by_id');
    ...
    public function getAdministrativeCosts($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $administrativeCosts = AdministrativeCost::with('createdBy', 'updatedBy', 'deletedBy', 'bank')
            ...
            if ($this->order_by == 'created_by_id') {
                $administrativeCosts->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $administrativeCosts->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $administrativeCosts->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $administrativeCosts->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $administrativeCosts->onlyTrashed();
            }

            if ($paginate) {
                return $administrativeCosts->paginate($this->per_page ?? 10);
            } else {
                return $administrativeCosts->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new AdministrativeCostExport($this->getAdministrativeCosts(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'administrativeCosts' => $this->getAdministrativeCosts(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = AdministrativeCost::whereDate('date', now());
        $yesterday = AdministrativeCost::whereDate('date', now()->subDay());

        $month = AdministrativeCost::whereMonth('date', now()->format('m'))->whereYear('date', now()->format('Y'));
        $lastMonth = AdministrativeCost::whereMonth('date', now()->subMonth()->format('m'))->whereYear('date', now()->subMonth()->format('Y'));

        $year = AdministrativeCost::whereYear('date', now()->format('Y'));
        $lastYear = AdministrativeCost::whereYear('date', now()->subYear()->format('Y'));

        $all = AdministrativeCost::query();
        $beforeThisYear = AdministrativeCost::whereYear('date', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'administrativeCosts' => $this->readyToLoad ? $this->getAdministrativeCosts() : collect(),
