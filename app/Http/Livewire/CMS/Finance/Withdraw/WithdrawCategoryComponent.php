...
use App\Exports\Finance\Withdraw\WithdrawCategoryExport;
use App\Http\Livewire\CMS\Component;
use App\Models\User;
use App\Models\WithdrawCategory;
use App\Services\WithdrawCategoryService;
use Illuminate\Support\Str;
use Maatwebsite\Excel\Facades\Excel;
use PDF;

class WithdrawCategoryComponent extends Component
{
    public $pageName;
    ...
    {
        $this->pageName = 'Withdraw Category';
    ...
    {
        if ($this->withdrawCategory) {
            $this->name = $this->name ?: $this->withdrawCategory->name;
            $this->description = $this->description ?: $this->withdrawCategory->description;
            $this->is_active = $this->is_active ?: $this->withdrawCategory->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->withdrawCategory = WithdrawCategory::withTrashed()->findOrFail($this->row);
            } else {
                $this->withdrawCategory = WithdrawCategory::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->withdrawCategory = WithdrawCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->withdrawCategory = WithdrawCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->withdrawCategory->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new WithdrawCategoryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new WithdrawCategoryService())->clone($this->validate(), $this->withdrawCategory);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new WithdrawCategoryService())->edit($this->withdrawCategory, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        $withdrawCategory = WithdrawCategory::find($id);

        if (! $withdrawCategory) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new WithdrawCategoryService())->active($withdrawCategory);

        $active = Str::slug(Str::active($withdrawCategory->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new WithdrawCategoryService())->delete($withdrawCategory);
    ...
    public function restore($id)
    {
        ...
        $withdrawCategory = WithdrawCategory::onlyTrashed()->find($id);

        if (! $withdrawCategory) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new WithdrawCategoryService())->restore($withdrawCategory);
    ...
    public function deletePermanent($id)
    {
        ...
        (new WithdrawCategoryService())->deletePermanent($withdrawCategory);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new WithdrawCategoryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new WithdrawCategoryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = WithdrawCategory::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = WithdrawCategory::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = WithdrawCategory::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getWithdrawCategorys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $withdrawCategories = WithdrawCategory::with('createdBy', 'updatedBy', 'deletedBy', 'withdraws')
            ...
            if ($this->order_by == 'created_by_id') {
                $withdrawCategories->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $withdrawCategories->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $withdrawCategories->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $withdrawCategories->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $withdrawCategories->onlyTrashed();
            }

            if ($paginate) {
                return $withdrawCategories->paginate($this->per_page ?? 10);
            } else {
                return $withdrawCategories->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new WithdrawCategoryExport($this->getWithdrawCategorys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'withdrawCategories' => $this->getWithdrawCategorys(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = WithdrawCategory::whereDate('created_at', now());
        $yesterday = WithdrawCategory::whereDate('created_at', now()->subDay());

        $month = WithdrawCategory::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = WithdrawCategory::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = WithdrawCategory::whereYear('created_at', now()->format('Y'));
        $lastYear = WithdrawCategory::whereYear('created_at', now()->subYear()->format('Y'));

        $all = WithdrawCategory::query();
        $beforeThisYear = WithdrawCategory::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'withdrawCategories' => $this->readyToLoad ? $this->getWithdrawCategorys() : collect(),
