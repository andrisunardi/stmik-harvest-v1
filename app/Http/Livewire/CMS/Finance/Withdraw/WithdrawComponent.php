...
namespace App\Http\Livewire\CMS\Finance\Withdraw;

use App\Exports\Finance\Withdraw\WithdrawExport;
...
class WithdrawComponent extends Component
{
    ...
    public function boot()
    {
        ...
        $this->pageSubCategoryName = 'Withdraw';
    ...
    {
        if ($this->withdraw) {
            $this->withdraw_category_id = $this->withdraw_category_id ?: $this->withdraw->withdraw_category_id;
            $this->bank_id = $this->bank_id ?: $this->withdraw->bank_id;
            $this->account_number = $this->account_number ?: $this->withdraw->account_number;
            $this->account_name = $this->account_name ?: $this->withdraw->account_name;
            $this->amount = $this->amount ?: $this->withdraw->amount;
            $this->date = $this->date ?: $this->withdraw->datetime?->format('Y-m-d');
            $this->time = $this->time ?: $this->withdraw->datetime?->format('H:i');
            $this->notes = $this->notes ?: $this->withdraw->notes;
            $this->is_active = $this->is_active ?: $this->withdraw->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->withdraw = Withdraw::withTrashed()->findOrFail($this->row);
            } else {
                $this->withdraw = Withdraw::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->withdraw = Withdraw::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->withdraw->id : null;

        return [
            'withdraw_category_id' => 'required|integer|numeric|exists:withdraw_categories,id',
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new WithdrawService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new WithdrawService())->clone($this->validate(), $this->withdraw);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new WithdrawService())->edit($this->withdraw, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new WithdrawService())->active($withdraw);

        $active = Str::slug(Str::active($withdraw->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new WithdrawService())->delete($withdraw);
    ...
    public function restore($id)
    {
        ...
        $withdraw = Withdraw::onlyTrashed()->find($id);

        if (! $withdraw) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new WithdrawService())->restore($withdraw);
    ...
    public function deletePermanent($id)
    {
        ...
        (new WithdrawService())->deletePermanent($withdraw);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new WithdrawService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new WithdrawService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Withdraw::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Withdraw::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Withdraw::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getWithdrawCategories()
    {
        return WithdrawCategory::active()->orderBy('name')->get();
    }
    ...
    public function getWithdraws($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $withdraws = Withdraw::with('createdBy', 'updatedBy', 'deletedBy', 'withdrawCategory', 'bank')
                ->when($this->code, fn ($q) => $q->where('code', 'LIKE', "%{$this->code}%"))
                ->when($this->withdraw_category_id, fn ($q) => $q->where('withdraw_category_id', $this->withdraw_category_id))
            ...
            if ($this->order_by == 'created_by_id') {
                $withdraws->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $withdraws->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $withdraws->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $withdraws->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $withdraws->onlyTrashed();
            }

            if ($paginate) {
                return $withdraws->paginate($this->per_page ?? 10);
            } else {
                return $withdraws->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new WithdrawExport($this->getWithdraws(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'withdraws' => $this->getWithdraws(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Withdraw::whereDate('datetime', now());
        $yesterday = Withdraw::whereDate('datetime', now()->subDay());

        $month = Withdraw::whereMonth('datetime', now()->format('m'))->whereYear('datetime', now()->format('Y'));
        $lastMonth = Withdraw::whereMonth('datetime', now()->subMonth()->format('m'))->whereYear('datetime', now()->subMonth()->format('Y'));

        $year = Withdraw::whereYear('datetime', now()->format('Y'));
        $lastYear = Withdraw::whereYear('datetime', now()->subYear()->format('Y'));

        $all = Withdraw::query();
        $beforeThisYear = Withdraw::whereYear('datetime', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'withdrawCategories' => $this->readyToLoad ? $this->getWithdrawCategories() : collect(),
            'banks' => $this->readyToLoad ? $this->getBanks() : collect(),
            'withdraws' => $this->readyToLoad ? $this->getWithdraws() : collect(),
