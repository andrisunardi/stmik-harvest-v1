...
use App\Exports\Finance\Deposit\DepositCategoryExport;
use App\Http\Livewire\CMS\Component;
use App\Models\DepositCategory;
use App\Models\User;
use App\Services\DepositCategoryService;
use Illuminate\Support\Str;
use Maatwebsite\Excel\Facades\Excel;
use PDF;

class DepositCategoryComponent extends Component
{
    public $pageName;
    ...
    {
        $this->pageName = 'Deposit Category';
    ...
    {
        if ($this->depositCategory) {
            $this->name = $this->name ?: $this->depositCategory->name;
            $this->description = $this->description ?: $this->depositCategory->description;
            $this->is_active = $this->is_active ?: $this->depositCategory->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->depositCategory = DepositCategory::withTrashed()->findOrFail($this->row);
            } else {
                $this->depositCategory = DepositCategory::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->depositCategory = DepositCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->depositCategory = DepositCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->depositCategory->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new DepositCategoryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new DepositCategoryService())->clone($this->validate(), $this->depositCategory);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new DepositCategoryService())->edit($this->depositCategory, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new DepositCategoryService())->active($depositCategory);

        $active = Str::slug(Str::active($depositCategory->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new DepositCategoryService())->delete($depositCategory);
    ...
    public function restore($id)
    {
        ...
        $depositCategory = DepositCategory::onlyTrashed()->find($id);

        if (! $depositCategory) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new DepositCategoryService())->restore($depositCategory);
    ...
    public function deletePermanent($id)
    {
        ...
        (new DepositCategoryService())->deletePermanent($depositCategory);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new DepositCategoryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new DepositCategoryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = DepositCategory::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = DepositCategory::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = DepositCategory::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getDepositCategorys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $depositCategories = DepositCategory::with('createdBy', 'updatedBy', 'deletedBy', 'deposits')
            ...
            if ($this->order_by == 'created_by_id') {
                $depositCategories->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $depositCategories->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $depositCategories->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $depositCategories->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $depositCategories->onlyTrashed();
            }

            if ($paginate) {
                return $depositCategories->paginate($this->per_page ?? 10);
            } else {
                return $depositCategories->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new DepositCategoryExport($this->getDepositCategorys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'depositCategories' => $this->getDepositCategorys(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = DepositCategory::whereDate('created_at', now());
        $yesterday = DepositCategory::whereDate('created_at', now()->subDay());

        $month = DepositCategory::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = DepositCategory::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = DepositCategory::whereYear('created_at', now()->format('Y'));
        $lastYear = DepositCategory::whereYear('created_at', now()->subYear()->format('Y'));

        $all = DepositCategory::query();
        $beforeThisYear = DepositCategory::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'depositCategories' => $this->readyToLoad ? $this->getDepositCategorys() : collect(),
