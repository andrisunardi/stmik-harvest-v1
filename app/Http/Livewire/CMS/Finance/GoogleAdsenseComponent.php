...
use App\Exports\Finance\GoogleAdsenseExport;
...
class GoogleAdsenseComponent extends Component
{
    ...
    {
        $this->pageName = 'Google Adsense';
        $this->pageTitle = trans('index.'.Str::snake($this->pageName));
        $this->pageSlug = Str::slug($this->pageName);
        $this->pageIcon = 'fas fa-money-bill-wave';
    ...
    {
        if ($this->googleAdsense) {
            $this->bank_id = $this->bank_id ?: $this->googleAdsense->bank_id;
            $this->account_number = $this->account_number ?: $this->googleAdsense->account_number;
            $this->account_name = $this->account_name ?: $this->googleAdsense->account_name;
            $this->amount = $this->amount ?: $this->googleAdsense->amount;
            $this->date = $this->date ?: $this->googleAdsense->datetime?->format('Y-m-d');
            $this->time = $this->time ?: $this->googleAdsense->datetime?->format('H:i');
            $this->notes = $this->notes ?: $this->googleAdsense->notes;
            $this->is_active = $this->is_active ?: $this->googleAdsense->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->googleAdsense = GoogleAdsense::withTrashed()->findOrFail($this->row);
            } else {
                $this->googleAdsense = GoogleAdsense::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->googleAdsense = GoogleAdsense::find($id);
    ...
    public function view($id)
    {
        ...
        $this->googleAdsense = GoogleAdsense::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->googleAdsense->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new GoogleAdsenseService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new GoogleAdsenseService())->clone($this->validate(), $this->googleAdsense);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new GoogleAdsenseService())->edit($this->googleAdsense, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new GoogleAdsenseService())->active($googleAdsense);

        $active = Str::slug(Str::active($googleAdsense->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new GoogleAdsenseService())->delete($googleAdsense);
    ...
    public function restore($id)
    {
        ...
        $googleAdsense = GoogleAdsense::onlyTrashed()->find($id);

        if (! $googleAdsense) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new GoogleAdsenseService())->restore($googleAdsense);
    ...
    public function deletePermanent($id)
    {
        ...
        (new GoogleAdsenseService())->deletePermanent($googleAdsense);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new GoogleAdsenseService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new GoogleAdsenseService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = GoogleAdsense::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = GoogleAdsense::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = GoogleAdsense::groupBy('deleted_by_id')->pluck('deleted_by_id');
    ...
    public function getGoogleAdsenses($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $googleAdsenses = GoogleAdsense::with('createdBy', 'updatedBy', 'deletedBy', 'user', 'bank')
            ...
            if ($this->order_by == 'created_by_id') {
                $googleAdsenses->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $googleAdsenses->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $googleAdsenses->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $googleAdsenses->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $googleAdsenses->onlyTrashed();
            }

            if ($paginate) {
                return $googleAdsenses->paginate($this->per_page ?? 10);
            } else {
                return $googleAdsenses->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new GoogleAdsenseExport($this->getGoogleAdsenses(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'googleAdsenses' => $this->getGoogleAdsenses(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = GoogleAdsense::whereDate('datetime', now());
        $yesterday = GoogleAdsense::whereDate('datetime', now()->subDay());

        $month = GoogleAdsense::whereMonth('datetime', now()->format('m'))->whereYear('datetime', now()->format('Y'));
        $lastMonth = GoogleAdsense::whereMonth('datetime', now()->subMonth()->format('m'))->whereYear('datetime', now()->subMonth()->format('Y'));

        $year = GoogleAdsense::whereYear('datetime', now()->format('Y'));
        $lastYear = GoogleAdsense::whereYear('datetime', now()->subYear()->format('Y'));

        $all = GoogleAdsense::query();
        $beforeThisYear = GoogleAdsense::whereYear('datetime', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'googleAdsenses' => $this->readyToLoad ? $this->getGoogleAdsenses() : collect(),
