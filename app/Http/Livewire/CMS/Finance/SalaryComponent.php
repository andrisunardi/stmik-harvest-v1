...
use App\Exports\Finance\SalaryExport;
...
class SalaryComponent extends Component
{
    ...
    {
        if ($this->salary) {
            $this->user_id = $this->user_id ?: $this->salary->user_id;
            $this->bank_id = $this->bank_id ?: $this->salary->bank_id;
            $this->account_number = $this->account_number ?: $this->salary->account_number;
            $this->account_name = $this->account_name ?: $this->salary->account_name;
            $this->amount = $this->amount ?: $this->salary->amount;
            $this->date = $this->date ?: $this->salary->datetime?->format('Y-m-d');
            $this->time = $this->time ?: $this->salary->datetime?->format('H:i');
            $this->notes = $this->notes ?: $this->salary->notes;
            $this->is_active = $this->is_active ?: $this->salary->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->salary = Salary::withTrashed()->findOrFail($this->row);
            } else {
                $this->salary = Salary::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->salary = Salary::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->salary->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new SalaryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new SalaryService())->clone($this->validate(), $this->salary);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new SalaryService())->edit($this->salary, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new SalaryService())->active($salary);

        $active = Str::slug(Str::active($salary->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new SalaryService())->delete($salary);
    ...
    public function restore($id)
    {
        ...
        (new SalaryService())->restore($salary);
    ...
    public function deletePermanent($id)
    {
        ...
        (new SalaryService())->deletePermanent($salary);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new SalaryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new SalaryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Salary::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Salary::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Salary::groupBy('deleted_by_id')->pluck('deleted_by_id');
    ...
    public function getSalarys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $salaries = Salary::with('createdBy', 'updatedBy', 'deletedBy', 'user', 'bank')
            ...
            if ($this->order_by == 'created_by_id') {
                $salaries->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $salaries->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $salaries->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $salaries->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $salaries->onlyTrashed();
            }

            if ($paginate) {
                return $salaries->paginate($this->per_page ?? 10);
            } else {
                return $salaries->get();
            }
        }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new SalaryExport($this->getSalarys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'salaries' => $this->getSalarys(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Salary::whereDate('datetime', now());
        $yesterday = Salary::whereDate('datetime', now()->subDay());

        $month = Salary::whereMonth('datetime', now()->format('m'))->whereYear('datetime', now()->format('Y'));
        $lastMonth = Salary::whereMonth('datetime', now()->subMonth()->format('m'))->whereYear('datetime', now()->subMonth()->format('Y'));

        $year = Salary::whereYear('datetime', now()->format('Y'));
        $lastYear = Salary::whereYear('datetime', now()->subYear()->format('Y'));

        $all = Salary::query();
        $beforeThisYear = Salary::whereYear('datetime', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'salaries' => $this->readyToLoad ? $this->getSalarys() : collect(),
