...
use App\Exports\Finance\BankExport;
...
class BankComponent extends Component
{
    ...
    {
        if ($this->bank) {
            $this->code = $this->code ?: $this->bank->code;
            $this->name = $this->name ?: $this->bank->name;
            $this->is_active = $this->is_active ?: $this->bank->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->bank = Bank::withTrashed()->findOrFail($this->row);
            } else {
                $this->bank = Bank::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->bank = Bank::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->bank->id : null;

        return [
            'code' => "required|max:3|unique:{$this->pageTable},code,{$id}",
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new BankService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new BankService())->clone($this->validate(), $this->bank);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new BankService())->edit($this->bank, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new BankService())->active($bank);

        $active = Str::slug(Str::active($bank->is_active), '_');
    ...
    public function deletePermanent($id)
    {
        ...
        (new BankService())->deletePermanent($bank);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new BankService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new BankService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Bank::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Bank::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Bank::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getBanks($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $banks = Bank::with('createdBy', 'updatedBy', 'deletedBy', 'administrativeCosts', 'bankInterests', 'buyAdvertisements', 'buyDomainHostings', 'buyEndorses', 'buyInternets', 'buyItems', 'buyPhoneCredits', 'googleAdsenses', 'loyalties', 'payments', 'salaries', 'users')
            ...
            if ($this->order_by == 'created_by_id') {
                $banks->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $banks->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $banks->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $banks->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $banks->paginate($this->per_page ?? 10);
            } else {
                return $banks->get();
            }
        }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new BankExport($this->getBanks(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'banks' => $this->getBanks(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Bank::whereDate('created_at', now());
        $yesterday = Bank::whereDate('created_at', now()->subDay());

        $month = Bank::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = Bank::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = Bank::whereYear('created_at', now()->format('Y'));
        $lastYear = Bank::whereYear('created_at', now()->subYear()->format('Y'));

        $all = Bank::query();
        $beforeThisYear = Bank::whereYear('created_at', '<', now()->format('Y'));
