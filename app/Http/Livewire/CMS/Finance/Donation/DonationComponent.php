...
namespace App\Http\Livewire\CMS\Finance\Donation;

use App\Exports\Finance\Donation\DonationExport;
...
class DonationComponent extends Component
{
    ...
    {
        $this->pageName = 'Donation';
        $this->pageTitle = trans('index.'.Str::snake($this->pageName));
        $this->pageSlug = Str::slug($this->pageName);
        $this->pageIcon = 'fas fa-donate';
        ...
        $this->pageSubCategoryName = 'Donation';
    ...
    {
        if ($this->donation) {
            $this->donation_category_id = $this->donation_category_id ?: $this->donation->donation_category_id;
            $this->bank_id = $this->bank_id ?: $this->donation->bank_id;
            $this->account_number = $this->account_number ?: $this->donation->account_number;
            $this->account_name = $this->account_name ?: $this->donation->account_name;
            $this->amount = $this->amount ?: $this->donation->amount;
            $this->date = $this->date ?: $this->donation->datetime?->format('Y-m-d');
            $this->time = $this->time ?: $this->donation->datetime?->format('H:i');
            $this->notes = $this->notes ?: $this->donation->notes;
            $this->is_active = $this->is_active ?: $this->donation->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->donation = Donation::withTrashed()->findOrFail($this->row);
            } else {
                $this->donation = Donation::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->donation = Donation::find($id);
    ...
    public function view($id)
    {
        ...
        $this->donation = Donation::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->donation->id : null;

        return [
            'donation_category_id' => 'required|integer|numeric|exists:donation_categories,id',
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new DonationService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new DonationService())->clone($this->validate(), $this->donation);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new DonationService())->edit($this->donation, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new DonationService())->active($donation);

        $active = Str::slug(Str::active($donation->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new DonationService())->delete($donation);
    ...
    public function restore($id)
    {
        ...
        (new DonationService())->restore($donation);
    ...
    public function deletePermanent($id)
    {
        ...
        (new DonationService())->deletePermanent($donation);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new DonationService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new DonationService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Donation::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Donation::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Donation::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getDonationCategories()
    {
        return DonationCategory::active()->orderBy('name')->get();
    }
    ...
    public function getDonations($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $donations = Donation::with('createdBy', 'updatedBy', 'deletedBy', 'donationCategory', 'bank')
                ->when($this->code, fn ($q) => $q->where('code', 'LIKE', "%{$this->code}%"))
                ->when($this->donation_category_id, fn ($q) => $q->where('donation_category_id', $this->donation_category_id))
            ...
            if ($this->order_by == 'created_by_id') {
                $donations->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $donations->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $donations->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $donations->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $donations->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new DonationExport($this->getDonations(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'donations' => $this->getDonations(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Donation::whereDate('datetime', now());
        $yesterday = Donation::whereDate('datetime', now()->subDay());

        $month = Donation::whereMonth('datetime', now()->format('m'))->whereYear('datetime', now()->format('Y'));
        $lastMonth = Donation::whereMonth('datetime', now()->subMonth()->format('m'))->whereYear('datetime', now()->subMonth()->format('Y'));

        $year = Donation::whereYear('datetime', now()->format('Y'));
        $lastYear = Donation::whereYear('datetime', now()->subYear()->format('Y'));

        $all = Donation::query();
        $beforeThisYear = Donation::whereYear('datetime', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'donationCategories' => $this->readyToLoad ? $this->getDonationCategories() : collect(),
            'banks' => $this->readyToLoad ? $this->getBanks() : collect(),
            'donations' => $this->readyToLoad ? $this->getDonations() : collect(),
