...
use App\Exports\Finance\Donation\DonationCategoryExport;
...
class DonationCategoryComponent extends Component
{
    public $pageName;
    ...
    {
        $this->pageName = 'Donation Category';
    ...
    public function resetForm()
    {
        if ($this->donationCategory) {
            $this->name = $this->name ?: $this->donationCategory->name;
            $this->description = $this->description ?: $this->donationCategory->description;
            $this->is_active = $this->is_active ?: $this->donationCategory->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->donationCategory = DonationCategory::withTrashed()->findOrFail($this->row);
            } else {
                $this->donationCategory = DonationCategory::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->donationCategory = DonationCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->donationCategory = DonationCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->donationCategory->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new DonationCategoryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new DonationCategoryService())->clone($this->validate(), $this->donationCategory);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new DonationCategoryService())->edit($this->donationCategory, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        $donationCategory = DonationCategory::find($id);

        if (! $donationCategory) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new DonationCategoryService())->active($donationCategory);

        $active = Str::slug(Str::active($donationCategory->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new DonationCategoryService())->delete($donationCategory);
    ...
    public function restore($id)
    {
        ...
        $donationCategory = DonationCategory::onlyTrashed()->find($id);

        if (! $donationCategory) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new DonationCategoryService())->restore($donationCategory);
    ...
    public function deletePermanent($id)
    {
        ...
        (new DonationCategoryService())->deletePermanent($donationCategory);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new DonationCategoryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new DonationCategoryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = DonationCategory::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = DonationCategory::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = DonationCategory::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getDonationCategorys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $donationCategories = DonationCategory::with('createdBy', 'updatedBy', 'deletedBy', 'donations')
            ...
            if ($this->order_by == 'created_by_id') {
                $donationCategories->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $donationCategories->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $donationCategories->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $donationCategories->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $donationCategories->onlyTrashed();
            }

            if ($paginate) {
                return $donationCategories->paginate($this->per_page ?? 10);
            } else {
                return $donationCategories->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new DonationCategoryExport($this->getDonationCategorys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'donationCategories' => $this->getDonationCategorys(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = DonationCategory::whereDate('created_at', now());
        $yesterday = DonationCategory::whereDate('created_at', now()->subDay());

        $month = DonationCategory::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = DonationCategory::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = DonationCategory::whereYear('created_at', now()->format('Y'));
        $lastYear = DonationCategory::whereYear('created_at', now()->subYear()->format('Y'));

        $all = DonationCategory::query();
        $beforeThisYear = DonationCategory::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'donationCategories' => $this->readyToLoad ? $this->getDonationCategorys() : collect(),
