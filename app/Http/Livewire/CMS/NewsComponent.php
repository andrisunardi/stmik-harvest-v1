...
class NewsComponent extends Component
{
    ...
    {
        $this->active = $this->news->active;
        $this->news_category = $this->news->news_category?->id;
        $this->name = $this->news->name;
        $this->name_id = $this->news->name_id;
        $this->description = $this->news->description;
        $this->description_id = $this->news->description_id;
        $this->date = $this->news->date;
        $this->tag = $this->news->tag;
        $this->tag_id = $this->news->tag_id;
    }
    ...
    public function updatingNewsCategory()
    {
        $this->resetPage();
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != "index" || $this->menu_type != "trash")) {
            if ($this->menu_type == "view") {
                $this->news = News::withTrashed()->find($this->row);
            } else {
                $this->news = News::find($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->news = News::withTrashed()->find($id);
    ...
    {
        $id = $this->menu_type == "edit" ? $this->news->id : null;

        return [
            "active"            => "required",
            "news_category"     => "required|exists:news_category,id",
    ...
    public function submit()
    {
        ...
        $this->news->news_category_id = $this->news_category;
        $this->news->name = $this->name;
        $this->news->name_id = $this->name_id;
        $this->news->description = Str::of(htmlspecialchars($this->description))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->news->description_id = Str::of(htmlspecialchars($this->description_id))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->news->date = $this->date;
        $this->news->tag = $this->tag;
        $this->news->tag_id = $this->tag_id;
        ...
        if ($this->image) {
            ...
            $this->news->image = date("YmdHis") . ".{$this->image->getClientOriginalExtension()}";
            $this->image->storePubliclyAs($this->menu_slug, $this->news->image, "images");
        } else {
            if ($this->menu_type == "clone") {
                if ($image && File::exists(public_path() . "/images/{$this->menu_slug}/{$image}")) {
                    $this->news->image = date("YmdHis") . "." . explode(".", $image)[1];
                    File::copy(public_path() . "/images/{$this->menu_slug}/{$image}", public_path() . "/images/{$this->menu_slug}/{$this->news->image}");
                }
            }
        }

        $this->news->slug = Str::slug($this->name);
        $this->news->save();
    ...
    {
        $this->news = News::onlyTrashed()->find($id);
    ...
    {
        $data_news = News::onlyTrashed()->get();
    ...
    public function getDataNews()
    {
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_news = News::query()
                ...
                ->when($this->news_category, function ($query) {
                    $query->where("news_category_id", $this->news_category);
                })
                ...
                if ($this->order_by == "created_by") {
                    $data_news->join("admin", "admin.id", "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "updated_by") {
                    $data_news->join("admin", "admin.id", "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "deleted_by") {
                    $data_news->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                    ...
                    $data_news->orderBy($this->order_by ?? "id", $this->sort_by ?? "desc");
                }

                if ($this->menu_type == "trash") {
                    $data_news->onlyTrashed();
                }

                return $data_news->paginate($this->per_page ?? 10);
        }
    }
