...
use App\Exports\Development\Portfolio\PortfolioCategoryExport;
...
class PortfolioCategoryComponent extends Component
{
    ...
    public function resetForm()
    {
        if ($this->portfolioCategory) {
            $this->name = $this->name ?: $this->portfolioCategory->name;
            $this->name_idn = $this->name_idn ?: $this->portfolioCategory->name_idn;
            $this->description = $this->description ?: $this->portfolioCategory->description;
            $this->description_idn = $this->description_idn ?: $this->portfolioCategory->description_idn;
            $this->is_active = $this->is_active ?: $this->portfolioCategory->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->portfolioCategory = PortfolioCategory::withTrashed()->findOrFail($this->row);
            } else {
                $this->portfolioCategory = PortfolioCategory::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->portfolioCategory = PortfolioCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->portfolioCategory = PortfolioCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->portfolioCategory->id : null;

        return [
            'name' => "required|max:50|unique:{$this->pageTable},name,{$id}",
            'name_idn' => "required|max:50|unique:{$this->pageTable},name_idn,{$id}",
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new PortfolioCategoryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new PortfolioCategoryService())->clone($this->validate(), $this->portfolioCategory);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new PortfolioCategoryService())->edit($this->portfolioCategory, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new PortfolioCategoryService())->active($portfolioCategory);

        $active = Str::slug(Str::active($portfolioCategory->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new PortfolioCategoryService())->delete($portfolioCategory);
    ...
    public function restore($id)
    {
        ...
        $portfolioCategory = PortfolioCategory::onlyTrashed()->find($id);

        if (! $portfolioCategory) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new PortfolioCategoryService())->restore($portfolioCategory);
    ...
    public function deletePermanent($id)
    {
        ...
        (new PortfolioCategoryService())->deletePermanent($portfolioCategory);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new PortfolioCategoryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new PortfolioCategoryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = PortfolioCategory::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = PortfolioCategory::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = PortfolioCategory::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getPortfolioCategorys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $portfolioCategories = PortfolioCategory::with('createdBy', 'updatedBy', 'deletedBy', 'portfolios')
            ...
            if ($this->order_by == 'created_by_id') {
                $portfolioCategories->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $portfolioCategories->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $portfolioCategories->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $portfolioCategories->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $portfolioCategories->onlyTrashed();
            }

            if ($paginate) {
                return $portfolioCategories->paginate($this->per_page ?? 10);
            } else {
                return $portfolioCategories->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new PortfolioCategoryExport($this->getPortfolioCategorys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'portfolioCategories' => $this->getPortfolioCategorys(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'portfolioCategories' => $this->readyToLoad ? $this->getPortfolioCategorys() : collect(),
