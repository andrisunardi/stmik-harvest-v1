...
class RegistrationComponent extends Component
{
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->registration = Registration::withTrashed()->findOrFail($this->row);
            } else {
                $this->registration = Registration::findOrFail($this->row);
            }
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new RegistrationService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new RegistrationService())->clone($this->validate(), $this->registration);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new RegistrationService())->edit($this->registration, $this->validate());
        }
    ...
    public function delete($id)
    {
        ...
        (new RegistrationService())->delete($registration);
    ...
    public function restore($id)
    {
        ...
        (new RegistrationService())->restore($registration);
    ...
    public function deletePermanent($id)
    {
        ...
        (new RegistrationService())->deletePermanent($registration);
    ...
    {
        $updated_by_id = Registration::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Registration::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getRegistrations($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            ...
            if ($this->order_by == 'created_by_id') {
                $registrations->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $registrations->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $registrations->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $registrations->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $registrations->onlyTrashed();
            }

            if ($paginate) {
                return $registrations->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new RegistrationExport($this->getRegistrations(paginate: false)), "{$this->pageName}.xlsx");
    }
