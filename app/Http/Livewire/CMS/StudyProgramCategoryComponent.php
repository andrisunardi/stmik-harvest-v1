...
class StudyProgramCategoryComponent extends Component
{
    use WithPagination, WithFileUploads;

    public $menu_name = "Study Program Category";
    public $menu_icon = "bi bi-tags";
    public $menu_slug = "study-program-category";
    public $menu_table = "study_program_category";
    ...
    public function resetFilter()
    {
        ...
        $this->study_program_category = null;
        $this->code = null;
        $this->name = null;
        $this->name_id = null;
    ...
    {
        $this->active = $this->study_program_category->active;
        $this->code = $this->study_program_category->code;
        $this->name = $this->study_program_category->name;
        $this->name_id = $this->study_program_category->name_id;
        $this->description = $this->study_program_category->description;
        $this->description_id = $this->study_program_category->description_id;
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != "index" || $this->menu_type != "trash")) {
            if ($this->menu_type == "view") {
                $this->study_program_category = StudyProgramCategory::withTrashed()->find($this->row);
            } else {
                $this->study_program_category = StudyProgramCategory::find($this->row);
            }

            if (!$this->study_program_category) {
    ...
    public function form($menu_type, $id)
    {
        ...
        if ($menu_type != "add" && $id) {
            $this->study_program_category = StudyProgramCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->study_program_category = StudyProgramCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->menu_type == "edit" ? $this->study_program_category->id : null;

        return [
            "active"            => "required",
            "code"              => "required|max:10|unique:{$this->menu_table},code,{$id}",
    ...
    public function submit()
    {
        ...
        if ($this->menu_type == "add" || $this->menu_type == "clone") {
            if ($this->menu_type == "clone") {
                $image = $this->study_program_category->image;
            }
            $this->study_program_category = new StudyProgramCategory();
            if (env("APP_ENV") != "testing") {
                DB::statement(DB::raw("ALTER TABLE {$this->menu_table} AUTO_INCREMENT = 1"));
            }
        }

        $this->study_program_category->active = $this->active;

        $this->study_program_category->code = $this->code;
        $this->study_program_category->name = $this->name;
        $this->study_program_category->name_id = $this->name_id;
        $this->study_program_category->description = Str::of(htmlspecialchars($this->description))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->study_program_category->description_id = Str::of(htmlspecialchars($this->description_id))->swap(["&lt;" => "<", "&gt;" => ">"]);

        if ($this->image) {
            if ($this->menu_type == "edit") {
                $this->study_program_category->deleteImage();
            }

            $this->study_program_category->image = date("YmdHis") . ".{$this->image->getClientOriginalExtension()}";
            $this->image->storePubliclyAs($this->menu_slug, $this->study_program_category->image, "images");
        } else {
            if ($this->menu_type == "clone") {
                if ($image && File::exists(public_path() . "/images/{$this->menu_slug}/{$image}")) {
                    $this->study_program_category->image = date("YmdHis") . "." . explode(".", $image)[1];
                    File::copy(public_path() . "/images/{$this->menu_slug}/{$image}", public_path() . "/images/{$this->menu_slug}/{$this->study_program_category->image}");
                }
            }
        }

        $this->study_program_category->slug = Str::slug($this->name);
        $this->study_program_category->save();
    ...
    public function active($id)
    {
        ...
        $this->study_program_category->active = true;
        $this->study_program_category->save();
        $this->study_program_category->refresh();
    ...
    public function nonActive($id)
    {
        ...
        $this->study_program_category->active = false;
        $this->study_program_category->save();
        $this->study_program_category->refresh();
    ...
    public function delete($id)
    {
        ...
        $this->study_program_category->delete();
        $this->study_program_category->refresh();
    ...
    {
        $this->study_program_category = StudyProgramCategory::onlyTrashed()->find($id);

        if (!$this->study_program_category) {
            return Session::flash("danger", trans("page.{$this->menu_name}") . " " . trans("message.not found or has been deleted"));
        }

        $this->study_program_category->restore();
        $this->study_program_category->refresh();
    ...
    public function deletePermanent($id)
    {
        ...
        $this->study_program_category->deleteImage();
        $this->study_program_category->forceDelete();
        $this->study_program_category->refresh();
    ...
    {
        StudyProgramCategory::onlyTrashed()->restore();
    ...
    {
        $data_study_program_category = StudyProgramCategory::onlyTrashed()->get();

        foreach ($data_study_program_category as $study_program_category) {
            $study_program_category->deleteImage();
            $study_program_category->forceDelete();
        }
    ...
    {
        $created_by = StudyProgramCategory::groupBy("created_by")->onlyActive()->pluck("created_by");
        return Admin::whereIn("id", $created_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataUpdatedBy()
    {
        $updated_by = StudyProgramCategory::groupBy("updated_by")->onlyActive()->pluck("updated_by");
        return Admin::whereIn("id", $updated_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataDeletedBy()
    {
        $deleted_by = StudyProgramCategory::groupBy("deleted_by")->onlyActive()->pluck("deleted_by");
    ...
    public function getDataStudyProgramCategory()
    {
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_study_program_category = StudyProgramCategory::query()
                ...
                if ($this->order_by == "created_by") {
                    $data_study_program_category->join("admin", "admin.id", "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "updated_by") {
                    $data_study_program_category->join("admin", "admin.id", "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "deleted_by") {
                    $data_study_program_category->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else {
                    $data_study_program_category->orderBy($this->order_by ?? "id", $this->sort_by ?? "desc");
                }

                if ($this->menu_type == "trash") {
                    $data_study_program_category->onlyTrashed();
                }

                return $data_study_program_category->paginate($this->per_page ?? 10);
        }
    }
