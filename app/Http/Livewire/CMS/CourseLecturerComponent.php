...
class CourseLecturerComponent extends Component
{
    use WithPagination;

    public $menu_name = "Course Lecturer";
    public $menu_icon = "bi bi-book-fill";
    public $menu_slug = "course-lecturer";
    public $menu_table = "course_lecturer";
    ...
    {
        $this->active = $this->course_lecturer->active;
        $this->course = $this->course_lecturer->course?->id;
        $this->lecturer = $this->course_lecturer->lecturer?->id;
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != "index" || $this->menu_type != "trash")) {
            if ($this->menu_type == "view") {
                $this->course_lecturer = CourseLecturer::withTrashed()->find($this->row);
            } else {
                $this->course_lecturer = CourseLecturer::find($this->row);
            }

            if (!$this->course_lecturer) {
    ...
    public function form($menu_type, $id)
    {
        ...
        if ($menu_type != "add" && $id) {
            $this->course_lecturer = CourseLecturer::find($id);
    ...
    public function view($id)
    {
        ...
        $this->course_lecturer = CourseLecturer::withTrashed()->find($id);
    ...
    public function submit()
    {
        ...
        if ($this->menu_type == "add" || $this->menu_type == "clone") {
            $this->course_lecturer = new CourseLecturer();
            if (env("APP_ENV") != "testing") {
                DB::statement(DB::raw("ALTER TABLE {$this->menu_table} AUTO_INCREMENT = 1"));
            }
        }

        $this->course_lecturer->active = $this->active;

        $this->course_lecturer->course_id = $this->course;
        $this->course_lecturer->lecturer_id = $this->lecturer;
        $this->course_lecturer->save();
    ...
    public function active($id)
    {
        ...
        $this->course_lecturer->active = true;
        $this->course_lecturer->save();
        $this->course_lecturer->refresh();
    ...
    public function nonActive($id)
    {
        ...
        $this->course_lecturer->active = false;
        $this->course_lecturer->save();
        $this->course_lecturer->refresh();
    ...
    {
        $this->course_lecturer = CourseLecturer::onlyTrashed()->find($id);

        if (!$this->course_lecturer) {
            return Session::flash("danger", trans("page.{$this->menu_name}") . " " . trans("message.not found or has been deleted"));
        }

        $this->course_lecturer->restore();
        $this->course_lecturer->refresh();
    ...
    public function deletePermanent($id)
    {
        ...
        $this->course_lecturer->forceDelete();
        $this->course_lecturer->refresh();
    ...
    {
        CourseLecturer::onlyTrashed()->restore();
    ...
    {
        CourseLecturer::onlyTrashed()->forceDelete();
    ...
    public function getDataCourseLecturer()
    {
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_course_lecturer = CourseLecturer::query()
                ...
                if ($this->created_by || $this->created_by == "0") {
                    $data_course_lecturer->where("created_by", $this->created_by);
                }
                if ($this->updated_by || $this->updated_by == "0") {
                    $data_course_lecturer->where("updated_by", $this->updated_by);
                }
                if ($this->deleted_by || $this->deleted_by == "0") {
                    $data_course_lecturer->where("deleted_by", $this->deleted_by);
                }
                if ($this->active || $this->active == "0") {
                    $data_course_lecturer->where("active", $this->active);
                }

                if ($this->order_by == "created_by") {
                    $data_course_lecturer->join("admin", "admin.id", "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "updated_by") {
                    $data_course_lecturer->join("admin", "admin.id", "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "deleted_by") {
                    $data_course_lecturer->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "course_id") {
                    $data_course_lecturer->join("course", "course.id", "{$this->course_table}.course_id")
                        ->select("{$this->course_table}.*", "course.name as course_name")
                        ->orderByRaw("course_name {$this->sort_by}");
                } else if ($this->order_by == "lecturer_id") {
                    $data_course_lecturer->join("lecturer", "lecturer.id", "{$this->lecturer_table}.lecturer_id")
                        ->select("{$this->lecturer_table}.*", "lecturer.name as lecturer_name")
                        ->orderByRaw("lecturer_name {$this->sort_by}");
                } else {
                    $data_course_lecturer->orderBy($this->order_by ?? "id", $this->sort_by ?? "desc");
                }

                if ($this->menu_type == "trash") {
                    $data_course_lecturer->onlyTrashed();
                }

                return $data_course_lecturer->paginate($this->per_page ?? 10);
        }
    }
    ...
    public function render()
    {
        return view("{$this->sub_domain}.livewire.{$this->menu_slug}.index", [
            ...
            "data_course_lecturer" => $this->getDataCourseLecturer(),
