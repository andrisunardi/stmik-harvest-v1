...
class FaqCategoryComponent extends Component
{
    use WithPagination;

    public $menu_name = "Faq Category";
    public $menu_icon = "bi bi-tags";
    public $menu_slug = "faq-category";
    public $menu_table = "faq_category";
    ...
    {
        $this->active = $this->faq_category->active;
        $this->name = $this->faq_category->name;
        $this->name_id = $this->faq_category->name_id;
        $this->description = $this->faq_category->description;
        $this->description_id = $this->faq_category->description_id;
    }
    ...
    public function mount()
    {
        ...
        if ($this->row && ($this->menu_type != "index" || $this->menu_type != "trash")) {
            if ($this->menu_type == "view") {
                $this->faq_category = FaqCategory::withTrashed()->find($this->row);
            } else {
                $this->faq_category = FaqCategory::find($this->row);
            }

            if (!$this->faq_category) {
    ...
    public function form($menu_type, $id)
    {
        ...
        if ($menu_type != "add" && $id) {
            $this->faq_category = FaqCategory::find($id);
    ...
    public function view($id)
    {
        ...
        $this->faq_category = FaqCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->menu_type == "edit" ? $this->faq_category->id : null;
    ...
    public function submit()
    {
        ...
        $this->faq_category->active = $this->active;

        $this->faq_category->name = $this->name;
        $this->faq_category->name_id = $this->name_id;
        $this->faq_category->description = Str::of(htmlspecialchars($this->description))->swap(["&lt;" => "<", "&gt;" => ">"]);
        $this->faq_category->description_id = Str::of(htmlspecialchars($this->description_id))->swap(["&lt;" => "<", "&gt;" => ">"]);
    ...
    public function nonActive($id)
    {
        ...
        $this->faq_category->active = false;
        $this->faq_category->save();
        $this->faq_category->refresh();
    ...
    {
        $this->faq_category = FaqCategory::onlyTrashed()->find($id);

        if (!$this->faq_category) {
            return Session::flash("danger", trans("page.{$this->menu_name}") . " " . trans("message.not found or has been deleted"));
        }

        $this->faq_category->restore();
        $this->faq_category->refresh();
    ...
    {
        FaqCategory::onlyTrashed()->restore();
    ...
    {
        FaqCategory::onlyTrashed()->forceDelete();
    ...
    {
        $created_by = FaqCategory::groupBy("created_by")->onlyActive()->pluck("created_by");
        return Admin::whereIn("id", $created_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataUpdatedBy()
    {
        $updated_by = FaqCategory::groupBy("updated_by")->onlyActive()->pluck("updated_by");
        return Admin::whereIn("id", $updated_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataDeletedBy()
    {
        $deleted_by = FaqCategory::groupBy("deleted_by")->onlyActive()->pluck("deleted_by");
    ...
    public function getDataFaqCategory()
    {
        if ($this->menu_type == "index" || $this->menu_type == "trash") {
            $data_faq_category = FaqCategory::query()
                ...
                if ($this->created_by || $this->created_by == "0") {
                    $data_faq_category->where("created_by", $this->created_by);
                }
                if ($this->updated_by || $this->updated_by == "0") {
                    $data_faq_category->where("updated_by", $this->updated_by);
                }
                if ($this->deleted_by || $this->deleted_by == "0") {
                    $data_faq_category->where("deleted_by", $this->deleted_by);
                }
                if ($this->active || $this->active == "0") {
                    $data_faq_category->where("active", $this->active);
                }

                if ($this->order_by == "created_by") {
                    $data_faq_category->join("admin", "admin.id", "{$this->menu_table}.created_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "updated_by") {
                    $data_faq_category->join("admin", "admin.id", "{$this->menu_table}.updated_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "deleted_by") {
                    $data_faq_category->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else {
                    $data_faq_category->orderBy($this->order_by ?? "id", $this->sort_by ?? "desc");
                }

                if ($this->menu_type == "trash") {
                    $data_faq_category->onlyTrashed();
                }

                return $data_faq_category->paginate($this->per_page ?? 10);
        }
    }
