...
use App\Exports\Other\NotificationExport;
...
class NotificationComponent extends Component
{
    ...
    public function resetForm()
    {
        if ($this->notification) {
            $this->name = $this->name ?: $this->notification->name;
            $this->icon = $this->icon ?: $this->notification->icon;
            $this->link = $this->link ?: $this->notification->link;
            $this->is_active = $this->is_active ?: $this->notification->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->notification = Notification::withTrashed()->findOrFail($this->row);
            } else {
                $this->notification = Notification::findOrFail($this->row);
            }
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new NotificationService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new NotificationService())->clone($this->validate(), $this->notification);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new NotificationService())->edit($this->notification, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new NotificationService())->active($notification);

        $active = Str::slug(Str::active($notification->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new NotificationService())->delete($notification);
    ...
    public function restore($id)
    {
        ...
        (new NotificationService())->restore($notification);
    ...
    public function deletePermanent($id)
    {
        ...
        (new NotificationService())->deletePermanent($notification);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new NotificationService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new NotificationService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Notification::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Notification::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Notification::groupBy('deleted_by_id')->pluck('deleted_by_id');
    ...
    public function getNotifications($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $notifications = Notification::with('createdBy', 'updatedBy', 'deletedBy')
            ...
            if ($this->order_by == 'created_by_id') {
                $notifications->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $notifications->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $notifications->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'user_id') {
                $notifications->join('users', 'users.id', "{$this->pageTable}.{$this->order_by}")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $notifications->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $notifications->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new NotificationExport($this->getNotifications(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'notifications' => $this->getNotifications(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Notification::whereDate('created_at', now());
        $yesterday = Notification::whereDate('created_at', now()->subDay());

        $month = Notification::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = Notification::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = Notification::whereYear('created_at', now()->format('Y'));
        $lastYear = Notification::whereYear('created_at', now()->subYear()->format('Y'));

        $all = Notification::query();
        $beforeThisYear = Notification::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'employees' => $this->readyToLoad ? $this->getEmployees() : collect(),
            'notifications' => $this->readyToLoad ? $this->getNotifications() : collect(),
