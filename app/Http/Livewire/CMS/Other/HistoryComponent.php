...
use App\Exports\Other\HistoryExport;
...
class HistoryComponent extends Component
{
    ...
    {
        if ($this->history) {
            $this->name = $this->name ?: $this->history->name;
            $this->name_idn = $this->name_idn ?: $this->history->name_idn;
            $this->description = $this->description ?: $this->history->description;
            $this->description_idn = $this->description_idn ?: $this->history->description_idn;
            $this->is_active = $this->is_active ?: $this->history->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->history = History::withTrashed()->findOrFail($this->row);
            } else {
                $this->history = History::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->history = History::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->history->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new HistoryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new HistoryService())->clone($this->validate(), $this->history);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new HistoryService())->edit($this->history, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new HistoryService())->active($history);

        $active = Str::slug(Str::active($history->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new HistoryService())->delete($history);
    ...
    public function restore($id)
    {
        ...
        $history = History::onlyTrashed()->find($id);

        if (! $history) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new HistoryService())->restore($history);
    ...
    public function deletePermanent($id)
    {
        ...
        (new HistoryService())->deletePermanent($history);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new HistoryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new HistoryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = History::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = History::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = History::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getHistorys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $histories = History::with('createdBy', 'updatedBy', 'deletedBy')
            ...
            if ($this->order_by == 'created_by_id') {
                $histories->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $histories->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $histories->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $histories->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $histories->onlyTrashed();
            }

            if ($paginate) {
                return $histories->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new HistoryExport($this->getHistorys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'histories' => $this->getHistorys(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = History::whereDate('created_at', now());
        $yesterday = History::whereDate('created_at', now()->subDay());

        $month = History::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = History::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = History::whereYear('created_at', now()->format('Y'));
        $lastYear = History::whereYear('created_at', now()->subYear()->format('Y'));

        $all = History::query();
        $beforeThisYear = History::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'histories' => $this->readyToLoad ? $this->getHistorys() : collect(),
