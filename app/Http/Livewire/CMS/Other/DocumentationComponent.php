...
use App\Exports\Other\DocumentationExport;
...
class DocumentationComponent extends Component
{
    ...
    public function resetForm()
    {
        if ($this->documentation) {
            $this->name = $this->name ?: $this->documentation->name;
            $this->description = $this->description ?: $this->documentation->description;
            $this->at = $this->at ?: $this->documentation->at;
            $this->date = $this->date ?: $this->documentation->datetime?->format('Y-m-d');
            $this->time = $this->time ?: $this->documentation->datetime?->format('H:i');
            $this->is_active = $this->is_active ?: $this->documentation->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->documentation = Documentation::withTrashed()->findOrFail($this->row);
            } else {
                $this->documentation = Documentation::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->documentation = Documentation::find($id);
    ...
    public function view($id)
    {
        ...
        $this->documentation = Documentation::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->documentation->id : null;

        return [
            'name' => "required|max:100|unique:{$this->pageTable},name,{$id}",
            'description' => 'nullable|max:65535',
            'at' => 'nullable|max:65535',
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new DocumentationService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new DocumentationService())->clone($this->validate(), $this->documentation);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new DocumentationService())->edit($this->documentation, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new DocumentationService())->active($documentation);

        $active = Str::slug(Str::active($documentation->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new DocumentationService())->delete($documentation);
    ...
    public function restore($id)
    {
        ...
        $documentation = Documentation::onlyTrashed()->find($id);

        if (! $documentation) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new DocumentationService())->restore($documentation);
    ...
    public function deletePermanent($id)
    {
        ...
        (new DocumentationService())->deletePermanent($documentation);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new DocumentationService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new DocumentationService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Documentation::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Documentation::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Documentation::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getDocumentations($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $documentations = Documentation::with('createdBy', 'updatedBy', 'deletedBy')
                ...
                ->when($this->at, fn ($q) => $q->where('at', 'LIKE', "%{$this->at}%"))
            ...
            if ($this->order_by == 'created_by_id') {
                $documentations->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $documentations->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $documentations->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $documentations->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $documentations->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new DocumentationExport($this->getDocumentations(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'documentations' => $this->getDocumentations(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Documentation::whereDate('datetime', now());
        $yesterday = Documentation::whereDate('datetime', now()->subDay());

        $month = Documentation::whereMonth('datetime', now()->format('m'))->whereYear('datetime', now()->format('Y'));
        $lastMonth = Documentation::whereMonth('datetime', now()->subMonth()->format('m'))->whereYear('datetime', now()->subMonth()->format('Y'));

        $year = Documentation::whereYear('datetime', now()->format('Y'));
        $lastYear = Documentation::whereYear('datetime', now()->subYear()->format('Y'));

        $all = Documentation::query();
        $beforeThisYear = Documentation::whereYear('datetime', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'documentations' => $this->readyToLoad ? $this->getDocumentations() : collect(),
