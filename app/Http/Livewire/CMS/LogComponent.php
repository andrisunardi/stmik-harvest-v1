...
class LogComponent extends Component
{
    ...
    public function updatingMenu()
    {
        $this->resetPage();
    }

    public function updatingRowMenu()
    {
        $this->resetPage();
    }
    ...
    {
        $created_by = Admin::groupBy("created_by")->onlyActive()->pluck("created_by");
        return Admin::whereIn("id", $created_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataUpdatedBy()
    {
        $updated_by = Admin::groupBy("updated_by")->onlyActive()->pluck("updated_by");
        return Admin::whereIn("id", $updated_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataDeletedBy()
    {
        $deleted_by = Admin::groupBy("deleted_by")->onlyActive()->pluck("deleted_by");
        return Admin::whereIn("id", $deleted_by)->onlyActive()->orderBy("name")->get();
    }

    public function getDataAdmin()
    {
        return Admin::onlyActive()->orderBy("name")->get();
    }

    public function getDataMenu()
    {
        return Menu::onlyActive()->orderBy("name")->get();
    }
            ...
            $data_log = Log::query()
                ...
                if ($this->order_by == "created_by") {
                    ...
                    $data_log->join("admin", "admin.id", "{$this->menu_table}.deleted_by")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "admin_id") {
                    $data_log->join("admin", "admin.id", "{$this->menu_table}.admin_id")
                        ->select("{$this->menu_table}.*", "admin.name as admin_name")
                        ->orderByRaw("admin_name {$this->sort_by}");
                } else if ($this->order_by == "menu_id") {
