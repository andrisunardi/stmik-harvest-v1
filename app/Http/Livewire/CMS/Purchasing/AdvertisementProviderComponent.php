...
use App\Exports\Purchasing\AdvertisementProviderExport;
use App\Http\Livewire\CMS\Component;
use App\Models\AdvertisementProvider;
use App\Models\User;
use App\Services\AdvertisementProviderService;
use Illuminate\Support\Str;
use Maatwebsite\Excel\Facades\Excel;
use PDF;

class AdvertisementProviderComponent extends Component
{
    public $pageName;
    ...
    {
        $this->pageName = 'Advertisement Provider';
    ...
    {
        if ($this->advertisementProvider) {
            $this->name = $this->name ?: $this->advertisementProvider->name;
            $this->link = $this->link ?: $this->advertisementProvider->link;
            $this->is_active = $this->is_active ?: $this->advertisementProvider->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->advertisementProvider = AdvertisementProvider::withTrashed()->findOrFail($this->row);
            } else {
                $this->advertisementProvider = AdvertisementProvider::findOrFail($this->row);
            }
    ...
    public function edit($id)
    {
        ...
        $this->advertisementProvider = AdvertisementProvider::find($id);
    ...
    public function view($id)
    {
        ...
        $this->advertisementProvider = AdvertisementProvider::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->advertisementProvider->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new AdvertisementProviderService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new AdvertisementProviderService())->clone($this->validate(), $this->advertisementProvider);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new AdvertisementProviderService())->edit($this->advertisementProvider, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        $advertisementProvider = AdvertisementProvider::find($id);

        if (! $advertisementProvider) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new AdvertisementProviderService())->active($advertisementProvider);

        $active = Str::slug(Str::active($advertisementProvider->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new AdvertisementProviderService())->delete($advertisementProvider);
    ...
    public function restore($id)
    {
        ...
        $advertisementProvider = AdvertisementProvider::onlyTrashed()->find($id);

        if (! $advertisementProvider) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new AdvertisementProviderService())->restore($advertisementProvider);
    ...
    public function deletePermanent($id)
    {
        ...
        (new AdvertisementProviderService())->deletePermanent($advertisementProvider);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new AdvertisementProviderService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new AdvertisementProviderService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = AdvertisementProvider::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = AdvertisementProvider::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = AdvertisementProvider::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getAdvertisementProviders($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $advertisementProviders = AdvertisementProvider::with('createdBy', 'updatedBy', 'deletedBy', 'buyAdvertisements')
            ...
            if ($this->order_by == 'created_by_id') {
                $advertisementProviders->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $advertisementProviders->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $advertisementProviders->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $advertisementProviders->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $advertisementProviders->onlyTrashed();
            }

            if ($paginate) {
                return $advertisementProviders->paginate($this->per_page ?? 10);
            } else {
                return $advertisementProviders->get();
            }
        }
    }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new AdvertisementProviderExport($this->getAdvertisementProviders(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'advertisementProviders' => $this->getAdvertisementProviders(paginate: false),
            'title' => $this->pageName,
        ])->output();
