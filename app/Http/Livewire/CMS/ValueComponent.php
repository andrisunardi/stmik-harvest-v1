...
class ValueComponent extends Component
{
    ...
    {
        if ($this->value) {
            $this->name = $this->name ?: $this->value->name;
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->value = Value::withTrashed()->findOrFail($this->row);
            } else {
                $this->value = Value::findOrFail($this->row);
            }
    ...
    {
        $id = $this->pageType == 'edit' ? $this->value->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new ValueService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new ValueService())->clone($this->validate(), $this->value);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new ValueService())->edit($this->value, $this->validate());
        }
    ...
    public function restore($id)
    {
        ...
        $value = Value::onlyTrashed()->find($id);

        if (! $value) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new ValueService())->restore($value);
    ...
    public function deletePermanent($id)
    {
        ...
        (new ValueService())->deletePermanent($value);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new ValueService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new ValueService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Value::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Value::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Value::groupBy('deleted_by_id')->pluck('deleted_by_id');
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new ValueExport($this->getValues(paginate: false)), "{$this->pageName}.xlsx");
    }
