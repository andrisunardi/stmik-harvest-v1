...
use App\Exports\Marketing\Blog\BlogCategoryExport;
...
class BlogCategoryComponent extends Component
{
    ...
    public function resetForm()
    {
        if ($this->blogCategory) {
            $this->name = $this->name ?: $this->blogCategory->name;
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->blogCategory = BlogCategory::withTrashed()->findOrFail($this->row);
            } else {
                $this->blogCategory = BlogCategory::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->blogCategory = BlogCategory::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->blogCategory->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new BlogCategoryService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new BlogCategoryService())->clone($this->validate(), $this->blogCategory);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new BlogCategoryService())->edit($this->blogCategory, $this->validate());
        }
    ...
    public function delete($id)
    {
        ...
        (new BlogCategoryService())->delete($blogCategory);
    ...
    public function restore($id)
    {
        ...
        (new BlogCategoryService())->restore($blogCategory);
    ...
    public function deletePermanent($id)
    {
        ...
        (new BlogCategoryService())->deletePermanent($blogCategory);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new BlogCategoryService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new BlogCategoryService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = BlogCategory::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = BlogCategory::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = BlogCategory::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getBlogCategorys($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $blogCategories = BlogCategory::with('createdBy', 'updatedBy', 'deletedBy', 'blogs')
            ...
            if ($this->order_by == 'created_by_id') {
                $blogCategories->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $blogCategories->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $blogCategories->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $blogCategories->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $blogCategories->onlyTrashed();
            }

            if ($paginate) {
                return $blogCategories->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new BlogCategoryExport($this->getBlogCategorys(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'blogCategories' => $this->readyToLoad ? $this->getBlogCategorys() : collect(),
