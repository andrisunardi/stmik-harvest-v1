...
use App\Exports\Marketing\PlatformExport;
...
class PlatformComponent extends Component
{
    ...
    public function boot()
    {
        ...
        $this->pageIcon = 'fas fa-circle-check';
    ...
    {
        if ($this->platform) {
            $this->name = $this->name ?: $this->platform->name;
            $this->description = $this->description ?: $this->platform->description;
            $this->is_active = $this->is_active ?: $this->platform->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->platform = Platform::withTrashed()->findOrFail($this->row);
            } else {
                $this->platform = Platform::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->platform = Platform::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->platform->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new PlatformService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new PlatformService())->clone($this->validate(), $this->platform);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new PlatformService())->edit($this->platform, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new PlatformService())->active($platform);

        $active = Str::slug(Str::active($platform->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new PlatformService())->delete($platform);
    ...
    public function restore($id)
    {
        ...
        $platform = Platform::onlyTrashed()->find($id);

        if (! $platform) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new PlatformService())->restore($platform);
    ...
    public function deletePermanent($id)
    {
        ...
        (new PlatformService())->deletePermanent($platform);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new PlatformService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new PlatformService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Platform::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Platform::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Platform::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getPlatforms($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $platforms = Platform::with('createdBy', 'updatedBy', 'deletedBy', 'users')
            ...
            if ($this->order_by == 'created_by_id') {
                $platforms->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $platforms->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $platforms->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $platforms->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }

            if ($this->pageType == 'trash') {
                $platforms->onlyTrashed();
            }

            if ($paginate) {
                return $platforms->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new PlatformExport($this->getPlatforms(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'platforms' => $this->getPlatforms(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Platform::whereDate('created_at', now());
        $yesterday = Platform::whereDate('created_at', now()->subDay());

        $month = Platform::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = Platform::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = Platform::whereYear('created_at', now()->format('Y'));
        $lastYear = Platform::whereYear('created_at', now()->subYear()->format('Y'));

        $all = Platform::query();
        $beforeThisYear = Platform::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'platforms' => $this->readyToLoad ? $this->getPlatforms() : collect(),
