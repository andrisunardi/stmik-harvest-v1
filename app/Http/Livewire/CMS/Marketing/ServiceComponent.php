...
use App\Exports\Marketing\ServiceExport;
...
class ServiceComponent extends Component
{
    ...
    {
        if ($this->service) {
            $this->code = $this->code ?: $this->service->code;
            $this->number = $this->number ?: $this->service->number;
            $this->name = $this->name ?: $this->service->name;
            $this->is_active = $this->is_active ?: $this->service->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->service = Service::withTrashed()->findOrFail($this->row);
            } else {
                $this->service = Service::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->service = Service::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->service->id : null;

        return [
            'number' => 'required|min:0|max:255|integer|numeric',
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new ServiceService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new ServiceService())->clone($this->validate(), $this->service);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new ServiceService())->edit($this->service, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new ServiceService())->active($service);

        $active = Str::slug(Str::active($service->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new ServiceService())->delete($service);
    ...
    public function restore($id)
    {
        ...
        (new ServiceService())->restore($service);
    ...
    public function deletePermanent($id)
    {
        ...
        (new ServiceService())->deletePermanent($service);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new ServiceService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new ServiceService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = Service::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Service::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Service::groupBy('deleted_by_id')->pluck('deleted_by_id');
    ...
    public function getServices($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $services = Service::with('createdBy', 'updatedBy', 'deletedBy')
                ...
                ->when($this->number, fn ($q) => $q->where('number', 'LIKE', "%{$this->number}%"))
            ...
            if ($this->order_by == 'created_by_id') {
                $services->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $services->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $services->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $services->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $services->paginate($this->per_page ?? 10);
            } else {
                return $services->get();
            }
        }
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new ServiceExport($this->getServices(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'services' => $this->getServices(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Service::whereDate('created_at', now());
        $yesterday = Service::whereDate('created_at', now()->subDay());

        $month = Service::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = Service::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = Service::whereYear('created_at', now()->format('Y'));
        $lastYear = Service::whereYear('created_at', now()->subYear()->format('Y'));

        $all = Service::query();
        $beforeThisYear = Service::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'services' => $this->readyToLoad ? $this->getServices() : collect(),
