...
use App\Exports\Marketing\PriceListExport;
...
class PriceListComponent extends Component
{
    ...
    {
        $this->pageName = 'Price List';
        $this->pageTitle = trans('index.'.Str::snake($this->pageName));
        $this->pageSlug = Str::slug($this->pageName);
        $this->pageIcon = 'fas fa-list-ol';
    ...
    {
        if ($this->priceList) {
            $this->name = $this->name ?: $this->priceList->name;
            $this->name_idn = $this->name_idn ?: $this->priceList->name_idn;
            $this->description = $this->description ?: $this->priceList->description;
            $this->description_idn = $this->description_idn ?: $this->priceList->description_idn;
            $this->price = $this->price ?: $this->priceList->price;
            $this->price_idn = $this->price_idn ?: $this->priceList->price_idn;
            $this->is_active = $this->is_active ?: $this->priceList->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->priceList = PriceList::withTrashed()->findOrFail($this->row);
            } else {
                $this->priceList = PriceList::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->priceList = PriceList::withTrashed()->find($id);
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new PriceListService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new PriceListService())->clone($this->validate(), $this->priceList);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new PriceListService())->edit($this->priceList, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new PriceListService())->active($priceList);

        $active = Str::slug(Str::active($priceList->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new PriceListService())->delete($priceList);
    ...
    public function restore($id)
    {
        ...
        $priceList = PriceList::onlyTrashed()->find($id);

        if (! $priceList) {
            return $this->alert('error', "{$this->pageName} ".trans('index.not_found_or_has_been_deleted'));
        }

        (new PriceListService())->restore($priceList);
    ...
    public function deletePermanent($id)
    {
        ...
        (new PriceListService())->deletePermanent($priceList);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new PriceListService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new PriceListService())->deletePermanentAll($ids);
    ...
    {
        $created_by_id = PriceList::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = PriceList::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = PriceList::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getPriceLists($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $priceLists = PriceList::with('createdBy', 'updatedBy', 'deletedBy')
                ...
                ->when($this->price_idn, fn ($q) => $q->where('price_idn', 'LIKE', "%{$this->price_idn}%"))
            ...
            if ($this->order_by == 'created_by_id') {
                $priceLists->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $priceLists->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $priceLists->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $priceLists->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $priceLists->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new PriceListExport($this->getPriceLists(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.pdf", [
            'priceLists' => $this->getPriceLists(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = PriceList::whereDate('created_at', now());
        $yesterday = PriceList::whereDate('created_at', now()->subDay());

        $month = PriceList::whereMonth('created_at', now()->format('m'))->whereYear('created_at', now()->format('Y'));
        $lastMonth = PriceList::whereMonth('created_at', now()->subMonth()->format('m'))->whereYear('created_at', now()->subMonth()->format('Y'));

        $year = PriceList::whereYear('created_at', now()->format('Y'));
        $lastYear = PriceList::whereYear('created_at', now()->subYear()->format('Y'));

        $all = PriceList::query();
        $beforeThisYear = PriceList::whereYear('created_at', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSubCategorySlug}.{$this->pageSlug}.index", [
            ...
            'priceLists' => $this->readyToLoad ? $this->getPriceLists() : collect(),
