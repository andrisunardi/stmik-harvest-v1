...
use App\Exports\Marketing\PromotionExport;
...
class PromotionComponent extends Component
{
    ...
    {
        if ($this->promotion) {
            $this->status = $this->status ?: $this->promotion->status;
            $this->name = $this->name ?: $this->promotion->name;
            $this->description = $this->description ?: $this->promotion->description;
            $this->start_date = $this->start_date ?: $this->promotion->start?->format('Y-m-d');
            $this->start_time = $this->start_time ?: $this->promotion->start?->format('H:i');
            $this->end_date = $this->end_date ?: $this->promotion->end?->format('Y-m-d');
            $this->end_time = $this->end_time ?: $this->promotion->end?->format('H:i');
            $this->slug = $this->slug ?: $this->promotion->slug;
            $this->is_active = $this->is_active ?: $this->promotion->is_active;
        }
    ...
    public function mount()
    {
        ...
        if ($this->row && (! in_array($this->pageType, ['index', 'trash']))) {
            if ($this->pageType == 'view') {
                $this->promotion = Promotion::withTrashed()->findOrFail($this->row);
            } else {
                $this->promotion = Promotion::findOrFail($this->row);
            }
    ...
    public function view($id)
    {
        ...
        $this->promotion = Promotion::withTrashed()->find($id);
    ...
    {
        $id = $this->pageType == 'edit' ? $this->promotion->id : null;
    ...
    public function submit()
    {
        ...
        if ($this->pageType == 'add') {
            $action = 'added';
            (new PromotionService())->add($this->validate());
        }

        if ($this->pageType == 'clone') {
            $action = 'cloned';
            (new PromotionService())->clone($this->validate(), $this->promotion);
        }

        if ($this->pageType == 'edit') {
            $action = 'edited';
            (new PromotionService())->edit($this->promotion, $this->validate());
        }
    ...
    public function active($id)
    {
        ...
        (new PromotionService())->active($promotion);

        $active = Str::slug(Str::active($promotion->is_active), '_');
    ...
    public function delete($id)
    {
        ...
        (new PromotionService())->delete($promotion);
    ...
    public function restore($id)
    {
        ...
        (new PromotionService())->restore($promotion);
    ...
    public function deletePermanent($id)
    {
        ...
        (new PromotionService())->deletePermanent($promotion);
    ...
    public function restoreAll(array $ids = [])
    {
        ...
        (new PromotionService())->restoreAll($ids);
    ...
    public function deletePermanentAll(array $ids = [])
    {
        ...
        (new PromotionService())->deletePermanentAll($ids);

        $this->alert('success', trans('index.all').' '."{$this->pageName} ".trans('index.at_trash_has_been_deleted_permanently_successfully'));
    }

    public function getPromotionStatuses()
    {
        return PromotionStatus::cases();
    }

    public function getCreatedBies()
    {
        $created_by_id = Promotion::groupBy('created_by_id')->pluck('created_by_id');
    ...
    {
        $updated_by_id = Promotion::groupBy('updated_by_id')->pluck('updated_by_id');
    ...
    {
        $deleted_by_id = Promotion::groupBy('deleted_by_id')->pluck('deleted_by_id');

        return User::whereIn('id', $deleted_by_id)->active()->orderBy('name')->get();
    }

    public function getPromotions($paginate = true)
    {
        if (in_array($this->pageType, ['index', 'trash'])) {
            $promotions = Promotion::with('createdBy', 'updatedBy', 'deletedBy')
                ...
                ->when($this->start_date, fn ($q) => $q->whereDate('start', $this->start_date))
                ->when($this->start_time, fn ($q) => $q->whereTime('start', $this->start_time))
                ->when($this->end_date, fn ($q) => $q->whereDate('end', $this->end_date))
                ->when($this->end_time, fn ($q) => $q->whereTime('end', $this->end_time))
            ...
            if ($this->order_by == 'created_by_id') {
                $promotions->leftJoin('users', 'users.id', "{$this->pageTable}.created_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'updated_by_id') {
                $promotions->leftJoin('users', 'users.id', "{$this->pageTable}.updated_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } elseif ($this->order_by == 'deleted_by_id') {
                $promotions->leftJoin('users', 'users.id', "{$this->pageTable}.deleted_by_id")
                    ->select("{$this->pageTable}.*", 'users.name as user_name')
                    ->orderByRaw("user_name {$this->sort_by}");
            } else {
                $promotions->orderBy($this->order_by ?? 'id', $this->sort_by ?? 'desc');
            }
            ...
            if ($paginate) {
                return $promotions->paginate($this->per_page ?? 10);
            } else {
    ...
    public function exportToExcel()
    {
        ...
        return Excel::download(new PromotionExport($this->getPromotions(paginate: false)), "{$this->pageName}.xlsx");
    }
    ...
    public function exportToPdf()
    {
        ...
        $pdf = PDF::loadView("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.pdf", [
            'promotions' => $this->getPromotions(paginate: false),
            'title' => $this->pageName,
        ])->output();
    ...
    {
        $today = Promotion::whereDate('start', now());
        $yesterday = Promotion::whereDate('start', now()->subDay());

        $month = Promotion::whereMonth('start', now()->format('m'))->whereYear('start', now()->format('Y'));
        $lastMonth = Promotion::whereMonth('start', now()->subMonth()->format('m'))->whereYear('start', now()->subMonth()->format('Y'));

        $year = Promotion::whereYear('start', now()->format('Y'));
        $lastYear = Promotion::whereYear('start', now()->subYear()->format('Y'));

        $all = Promotion::query();
        $beforeThisYear = Promotion::whereYear('start', '<', now()->format('Y'));
    ...
    public function render()
    {
        return view("{$this->subDomain}.livewire.{$this->pageCategorySlug}.{$this->pageSlug}.index", [
            ...
            'promotionStatuses' => $this->readyToLoad ? $this->getPromotionStatuses() : collect(),
            'promotions' => $this->readyToLoad ? $this->getPromotions() : collect(),
