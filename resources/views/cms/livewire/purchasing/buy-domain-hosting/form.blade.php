<div>
    <form wire:submit.prevent="submit" role="form" autocomplete="off">
        ...
        <div class="row">
            <div class="form-group col-sm-6 mb-3">
                ...
                <div class="input-group">
                    <div class="input-group-text @if ($errors->any()) {{ $errors->has($input) ? "border-danger" : "border-success" }}@endif">
                        <span class="fas fa-th-large fa-fw @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif"></span>
                    </div>
                    <select
                        ...
                        @foreach ($portfolios as $portfolio)
                            <option value="{{ $portfolio->id }}" {{ $portfolio->id == $portfolio_id ? "selected" : null }}>{{ $portfolio->name }}</option>
                        @endforeach
                    </select>
                    @error($input)
            ...
            <div class="form-group col-sm-6 mb-3">
                @php $input = "domain_hosting_provider_id" @endphp
                <label for="{{ $input }}" class="form-label @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif">
                    {{ trans("validation.attributes.{$input}") }}
                    <span class="text-danger">*</span>
                </label>
                <div class="input-group">
                    ...
                    <select
                        ...
                        @foreach ($domainHostingProviders as $domainHostingProvider)
                            <option value="{{ $domainHostingProvider->id }}" {{ $domainHostingProvider->id == $domain_hosting_provider_id ? "selected" : null }}>{{ $domainHostingProvider->name }}</option>
                        @endforeach
                    </select>
                    @error($input)
                        <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="row">
            <div class="form-group col-sm-6 mb-3">
                @php $input = "payment_id" @endphp
                <label for="{{ $input }}" class="form-label @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif">
                    {{ trans("validation.attributes.{$input}") }}
                </label>
                <div class="input-group">
                    <div class="input-group-text @if ($errors->any()) {{ $errors->has($input) ? "border-danger" : "border-success" }}@endif">
                        <span class="fas fa-credit-card fa-fw @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif"></span>
                    </div>
                    <select
                        class="form-select select2 @if ($errors->any()) {{ $errors->has($input) ? "is-invalid" : "is-valid" }}@endif"
                        wire:model="{{ $input }}"
                        ...
                        @foreach ($payments as $payment)
                            <option value="{{ $payment->id }}" {{ $payment->id == $payment_id ? "selected" : null }}>{{ $payment->code }}</option>
                        @endforeach
                    </select>
                    @error($input)
                        <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="row">
            ...
            <div class="form-group col-sm-6 mb-3">
                ...
                @if ($image || ($pageType != "add" && $buyDomainHosting->checkImage()))
                    @if ($image ? $image->temporaryUrl() : $buyDomainHosting->checkImage())
                        <img draggable="false" class="img-fluid img-thumbnail rounded w-100 mt-3"
                            src="{{ $image ? $image->temporaryUrl() : $buyDomainHosting->assetImage() }}"
                            alt="{{ $image ? trans("index.temporary_url") : $buyDomainHosting->altImage() }}"
                            data-bs-toggle="tooltip"
                            data-bs-placement="top"
                            title="{{ trans("index.click_to_view") }}" />
                        <div class="mt-3">
                            <a draggable="false" class="btn btn-sm btn-primary" href="{{ $image ? $image->temporaryUrl() : $buyDomainHosting->assetImage() }}" target="_blank">
                                <i class="fas fa-eye me-1"></i>
                                {{ trans("index.view") }}
                            </a>
                            <a draggable="false" class="btn btn-sm btn-info text-white" href="{{ $image ? $image->temporaryUrl() : $buyDomainHosting->assetImage() }}" download>
