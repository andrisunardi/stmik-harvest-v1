    ...
    <form wire:submit.prevent="submit" role="form" autocomplete="off">
        ...
        <div class="row">
            <div class="form-group col-sm-6 mb-3">
                ...
                <label for="{{ $input }}" class="form-label @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif">
                    {{ trans("validation.attributes.{$input}") }}
                    @if ($pageType != "edit") <span class="text-danger">*</span> @endif
                </label>
                <div class="input-group">
                    <div class="input-group-text @if ($errors->any()) {{ $errors->has($input) ? "border-danger" : "border-success" }}@endif">
                        <span class="fas fa-lock fa-fw @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif"></span>
                    </div>
                    <input
                        class="form-control @if ($errors->any()) {{ $errors->has($input) ? "is-invalid" : "is-valid" }}@endif"
                        wire:model="{{ $input }}"
                    ...
                    @if ($image ? $image->temporaryUrl() : $user->checkImage())
                        ...
                        <div class="row mt-3">
                            <div class="col-6 col-md-auto">
                                <a draggable="false" class="btn btn-sm btn-primary w-100" href="{{ $image ? $image->temporaryUrl() : $user->assetImage() }}" target="_blank">
                                    <i class="fas fa-eye me-1"></i>
                                    {{ trans("index.view") }}
                                </a>
                            </div>
                            <div class="col-6 col-md-auto">
                                <a draggable="false" class="btn btn-sm btn-info text-white w-100" href="{{ $image ? $image->temporaryUrl() : $user->assetImage() }}" download>
        ...
        <div class="row">
            ...
            <div class="form-group col-sm-6 mb-3">
                ...
                <div class="input-group">
                    <div class="input-group-text @if ($errors->any()) {{ $errors->has($input) ? "border-danger" : "border-success" }}@endif">
                        <span class="fas fa-suitcase fa-fw @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif"></span>
                    </div>
                    <select
                        class="form-select select2 @if ($errors->any()) {{ $errors->has($input) ? "is-invalid" : "is-valid" }}@endif"
                        wire:model="{{ $input }}"
                        id="{{ $input }}"
                        data-placeholder="{{ trans("index.select") }} {{ trans("validation.attributes.{$input}") }}"
                        multiple>
                        <option value="">{{ trans("index.select") }} {{ trans("validation.attributes.{$input}") }}</option>
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" {{ in_array($role->id, $roles_id) ? "selected" : null }}>{{ $role->name }}</option>
                        @endforeach
                    </select>
                    @error($input)
                        <div class="invalid-feedback">{{ $message }}</div>
