    ...
    <form wire:submit.prevent="submit" role="form" autocomplete="off">
        ...
        <div class="row">
            ...
            <div class="form-group col-sm-6 mb-3">
                ...
                <div class="input-group">
                    <div class="input-group-text @if ($errors->any()) {{ $errors->has($input) ? "border-danger" : "border-success" }}@endif">
                        <span class="fas fa-shield fa-fw @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif"></span>
                    </div>
                    <input
                        class="form-control @if ($errors->any()) {{ $errors->has($input) ? "is-invalid" : "is-valid" }}@endif"
                        wire:model="{{ $input }}"
        ...
        <div class="row">
            <div class="form-group col-12 mb-3">
                @php $input = "permissions_id" @endphp
                <label for="{{ $input }}" class="form-label @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif">
                    {{ trans("validation.attributes.{$input}") }}
                </label>
                <div class="input-group">
                    <div class="input-group-text @if ($errors->any()) {{ $errors->has($input) ? "border-danger" : "border-success" }}@endif">
                        <span class="fas fa-key fa-fw @if ($errors->any()) {{ $errors->has($input) ? "text-danger" : "text-success" }}@endif"></span>
                    </div>
                    <select
                        ...
                        @foreach ($permissions as $permission)
                            <option value="{{ $permission->id }}" {{ in_array($permission->id, $permissions_id) ? "selected" : null }}>{{ $permission->name }}</option>
                        @endforeach
                    </select>
                    @error($input)
                        <div class="invalid-feedback">{{ $message }}</div>
