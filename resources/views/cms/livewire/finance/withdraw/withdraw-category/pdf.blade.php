        ...
        <main class="text-center">
            ...
            <table class="table table-bordered">
                <tr>
                    ...
                    <th class="text-center">{{ trans("index.total") }} {{ trans("index.withdraw") }}</th>
                </tr>
                @foreach($withdrawCategories as $withdrawCategory)
                <tr>
                    <td class="text-center">{{ $loop->iteration }}</td>
                    <td class="text-center">{{ $withdrawCategory->id }}</td>
                    <td class="text-center">{{ $withdrawCategory->code }}</td>
                    <td>{{ $withdrawCategory->name }}</td>
                    <td>{{ $withdrawCategory->description }}</td>
                    <td class="text-center">{{ trans("index." . Str::slug(Str::active($withdrawCategory->is_active), "_")) }}</td>
                    <td class="text-center">{{ $withdrawCategory->withdraws->count() }}</td>
                </tr>
                @endforeach
            </table>
        </main>
