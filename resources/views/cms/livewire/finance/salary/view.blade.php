<div>
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6 col-md-8 col-lg-9">
            @if ($salary->datetime)
                {{ $salary->datetime->format("l, H:i:s") }}<br class="d-md-none">
                {{ $salary->datetime->isoFormat("LL") }}<br class="d-md-none">
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6 col-md-8 col-lg-9">
            @if ($salary->created_at)
                {{ $salary->created_at->format("l, H:i:s") }}<br class="d-md-none">
                {{ $salary->created_at->isoFormat("LL") }}<br class="d-md-none">
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6 col-md-8 col-lg-9">
            @if ($salary->updated_at)
                {{ $salary->updated_at->format("l, H:i:s") }}<br class="d-md-none">
                {{ $salary->updated_at->isoFormat("LL") }}<br class="d-md-none">
        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6 col-md-8 col-lg-9">
                @if ($salary->deleted_at)
                    {{ $salary->deleted_at->format("l, H:i:s") }}<br class="d-md-none">
                    {{ $salary->deleted_at->isoFormat("LL") }}<br class="d-md-none">
        ...
        @if ($salary->trashed())
            @can("{$pageName} Restore")
                <div class="col-12 col-sm-auto mt-3 mt-sm-0">
                    <button class="btn btn-sm btn-success w-100" type="button" data-bs-toggle="modal" data-bs-target="#restore-{{ $salary->id }}">
            ...
            @can("{$pageName} Delete Permanent")
                <div class="col-12 col-sm-auto mt-3 mt-sm-0">
                    <button class="btn btn-sm btn-danger w-100" type="button" data-bs-toggle="modal" data-bs-target="#delete-permanent-{{ $salary->id }}">
        ...
        @else
            ...
            @can("{$pageName} Edit")
                <div class="col-6 col-sm-auto mt-3 mt-sm-0">
                    <button class="btn btn-sm btn-{{ $salary->is_active ? "danger" : "success" }} w-100" wire:click="active({{ $salary->id }})" wire:loading.attr="disabled">
                        <i class="fas fa-{{ $salary->is_active ? "times" : "check" }}-circle me-1"></i>
                        {{ $salary->is_active ? trans("index.inactive") : trans("index.active") }}
                        <div wire:loading wire:target="active({{ $salary->id }})"><i class="fas fa-spinner fa-spin"></i></div>
                    </button>
                </div>
            @endcan
            ...
            @can("{$pageName} Clone")
                <div class="col-6 col-sm-auto mt-3 mt-sm-0">
                    <button class="btn btn-sm btn-info text-white w-100" wire:click="clone({{ $salary->id }})" wire:loading.attr="disabled">
            ...
            @can("{$pageName} Edit")
                <div class="col-6 col-sm-auto mt-3 mt-sm-0">
                    <button class="btn btn-sm btn-success w-100" wire:click="edit({{ $salary->id }})" wire:loading.attr="disabled">
            ...
            @can("{$pageName} Delete")
                <div class="col-6 col-sm-auto mt-3 mt-sm-0">
                    <button class="btn btn-sm btn-danger w-100" type="button" data-bs-toggle="modal" data-bs-target="#delete-{{ $salary->id }}">
                        <i class="fas fa-trash me-1"></i>
                        {{ trans("index.delete") }}
                    </button>

                    <div class="modal fade" id="delete-{{ $salary->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="delete-{{ $salary->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h6 class="modal-title" id="delete-{{ $salary->id }}">{{ trans("index.delete") }} - {{ $pageTitle }}</h6>
                                    <button class="btn btn-close p-1 ms-auto" type="button" data-bs-dismiss="modal" aria-label="{{ trans("index.close") }}"></button>
                                </div>
                                <div class="modal-body">
