    ...
    <form wire:submit.prevent="submit" role="form" autocomplete="off">
        ...
        <div class="row">
            <div class="form-group col-sm-6 mb-3">
                @php $input = "payment_category_id" @endphp
                ...
                <div class="input-group">
                    ...
                    <select
                        ...
                        @foreach ($paymentCategories as $paymentCategory)
                            <option value="{{ $paymentCategory->id }}" {{ $paymentCategory->id == $payment_category_id ? "selected" : null }}>{{ $paymentCategory->name }}</option>
                        @endforeach
                    </select>
                    @error($input)
        ...
        <div class="row">
            ...
            <div class="form-group col-sm-6 mb-3">
                ...
                @if ($image || ($pageType != "add" && $payment->checkImage()))
                    @if ($image ? $image->temporaryUrl() : $payment->checkImage())
                        <img draggable="false" src="{{ $image ? $image->temporaryUrl() : $payment->assetImage() }}" class="w-100 mt-3" />
                        <div class="mt-3">
                            <a draggable="false" class="btn btn-sm btn-primary" href="{{ $image ? $image->temporaryUrl() : $payment->assetImage() }}" target="_blank">
                                <i class="fas fa-eye me-1"></i>
                                {{ trans("index.view") }}
                            </a>
                            <a draggable="false" class="btn btn-sm btn-info text-white" href="{{ $image ? $image->temporaryUrl() : $payment->assetImage() }}" download>
